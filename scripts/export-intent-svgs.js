 ...
 * Same post-processing as export-switchless-svgs.js (which must NOT be modified).
 */
...
const drawioFiles = fs.readdirSync(dir).filter(f =>
    (f.includes('compute-storage') || f.includes('4intent')) && f.endsWith('.drawio'));
...
console.log('\nAll intent diagram SVGs exported and post-processed!');
