...
- **Per-rack leaf switches (SC1b — all racks)**: "Per-Rack Leaf Switches" panel now covers **every rack for disaggregated deployments (Rack 1 through Rack N, 1–8 racks)**, not just racks 2+. Each rack gets its own collapsible card with editable ToR-A / ToR-B / BMC hostnames, site, leaf BGP ASN (auto-increments from 64789), loopback pair, Mgmt SVI IPs, **per-rack P2P underlay links to Border1 / Border2 (/30)**, and **per-rack iBGP peering IPs**. For disaggregated deployments, the legacy single-rack Rack 1 hostname/site fields in SC1 and the Rack 1 Loopback/P2P/iBGP fields in SC3 are hidden — everything rack-specific lives in SC1b. The shared Border Router BGP ASN remains in SC3.
- **Validator: Deployment Type profiles**: The "QA Validate Existing ToR Switch Configuration" panel now has a **Deployment Type** dropdown with five options: `Auto-detect from Designer`, `Hyperconverged — Switched`, `Hyperconverged — Switchless`, `Disaggregated — Fibre Channel SAN`, and `Disaggregated — iSCSI (evolving guidance)`. Each profile maps every QoS check to one of `required` / `recommended` / `skip` so the validator no longer falsely fails FC SAN and switchless deployments on PFC / ECN / ETS-storage. In HCI Switchless the ToR only carries Mgmt + Compute (storage + cluster NICs are back-to-back), so PFC, class-maps, ETS storage & cluster, ECN, system-qos, interface PFC, and interface qos-policy are all reported as **N/A** with explanatory notes. In Disaggregated FC SAN, storage is on the FC fabric so the Ethernet-side storage checks are N/A but Cluster CSV/LM QoS (class-map CoS 7, ETS cluster, system-qos, interface qos-policy) still applies. Disaggregated iSCSI demotes storage-side checks from `fail` to `warn` as a placeholder — authoritative guidance is tracked for a future release. Results now show a grey `ℹ️ N not applicable` count alongside pass/warn/fail.
- **Per-rack leaf switches (SC1b)**: New "Per-Rack Leaf Switches" panel for disaggregated deployments with 2–8 racks. Each rack gets its own collapsible card with editable ToR-A / ToR-B / BMC hostnames, leaf BGP ASN (auto-increments from 64789), loopback pair, Mgmt SVI IPs, and site. The generator now emits up to **16 ToR configs + 8 BMC configs** (2 ToRs + 1 BMC per rack) in a single run, with labels `Rack N — ToR-A (hostname)`. Output tabs scroll horizontally when they exceed the container width.- **Editable Cluster Network names (DA4)**: Disaggregated "Cluster (CSV/LM) A" and "Cluster (CSV/LM) B" rows on DA4 now have a "Network Name" text input (default `Cluster Network 1` / `Cluster Network 2`). The name flows to the DA8 Adapter Mapping intent-zone titles, the DA8 Overrides card headings, and to ARM `sanNetworkList.clusterNetworkConfig.adapterIPConfig[*].name` (sanitized: spaces → hyphens, alphanumeric + hyphen only).
