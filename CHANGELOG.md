...
- **Agentic Retrieval sizing aligned with Microsoft Learn** (`sizer/sizer.js`, `tests/index.html`) — replaces the previous four-identical-worker approximation with three D8s_v3-equivalent CPU workers in every deployment, two A2/A16 embedding GPU workers for Combined and Knowledge modes, and no embedding workers for Agentic-only mode. The workload now exposes Combined, Knowledge, and Agentic deployment modes plus external, Foundry Local minimum, and Foundry Local production language-model endpoint choices. Local GPT-OSS-20B profiles add the documented dedicated CPU, memory, storage, GPU, and VRAM baseline. GPU selection is restricted to A2/A16, and insufficient VRAM is rejected.
...
- **Validation** — all **1,503 / 1,503** browser tests pass, including deployment-mode, endpoint-capacity, GPU-family, VRAM, legacy-import, schema-drift, and report contract coverage.
