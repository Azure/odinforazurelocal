...
- **New "Workloads" slide in the PowerPoint export** (`report/pptx-export.js`). When the configuration report was started from the Sizer (so the report has `sizerWorkloads` data), the generated PPTX now includes a dedicated Workloads slide — positioned right after Rack Configuration — with one bullet per workload (VM / AKS Arc / AVD) showing the headline spec plus a Subtotal sub-bullet (vCPUs · memory · storage). The slide auto-skips when no Sizer workload data is present, so non-Sizer reports are unaffected. Smoke-test slide count and assertions are unchanged.
