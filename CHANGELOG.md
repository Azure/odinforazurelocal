...
- **AUTO-scaled field state now survives F5 / Resume** in `getSizerState()` and `resumeSizerState()` (`sizer/sizer.js`). The in-memory `_autoScaledFields` Set and `_disaggAutoUpgraded` one-shot guard were not persisted, so after the user pressed F5 and clicked **Resume**, every AUTO badge was lost (including the cluster-type badge required by the auto-downgrade block). Symptom: removing a MANUAL override on Memory after Resume left the cluster on Disaggregated forever, instead of dropping back to Hyperconverged like it does in the same session before F5. The saved state now carries `autoScaledFields` (string array of field IDs) and `disaggAutoUpgraded` (boolean); Resume re-hydrates both before calling `calculateRequirements()`, which then re-renders the badges via the existing `previouslyAutoScaled` re-apply blocks and enables the symmetric auto-downgrade to fire as expected. Schema updated (`docs/json-schema/odin-sizer.schema.json`) — both fields are optional and backwards-compatible (absent → empty Set / false).
