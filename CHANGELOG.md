...
- **Design report fidelity improved** (`sizer/sizer.js`, `report/`, `docs/json-schema/odin-design.schema.json`, `tests/index.html`) — reports now use the title "Azure Local Instance | Design Configuration Report," identify Designer-only or Sizer-and-Designer workflow inputs with correct singular/plural wording, carry each workload's resolved GPU model and mode into HTML, Word, Markdown, and PowerPoint, match the Sizer's bold amber Advisory treatment, and label GPU racks as "GPU Enabled."
- **S2D volume guidance integrated with design reports** (`s2d-calc/`, `sizer/`, `report/`, `docs/json-schema/odin-design.schema.json`, `tests/index.html`) — Single Node, standard hyperconverged, and rack-aware Sizer results now show a compact maximum-volume calculation with the recommended volume count and a local S2D Calc link. The exact bounded result and derivation flow through Designer into HTML, Word, Markdown, and PowerPoint without recalculation; ALDO management and disaggregated designs omit it.
...
- **ToR configuration planning linked from design reports** (`report/`, `scripts/smoke-test-pptx.js`) — Host Networking includes an accessible information tooltip linking to the portable ToR Switch Configuration Generator & Validator for example Cisco and Dell configurations. The Physical Network Configuration PowerPoint slide embeds the same clickable link and reminds users to validate generated examples before deployment.
