...
- **Electricity-price annual cost now labelled `Annual (estimated) cost:`** in the Sizer's *Estimated Power, Heat & Rack Space per Instance* panel (`sizer/index.html`, `sizer/sizer.js`). The computed annual cost (e.g. `47,041/yr`) was previously rendered as a bare value with no label, which read ambiguously next to the `Input:`-labelled price box above it. The cost line is now wrapped in the same `.power-price-input-row` layout as the input row, with an `Annual (estimated) cost:` hint in the same small secondary-grey font as `Input:` — the two labels align vertically. The hint hides itself when the price field is empty / invalid (the row collapses back to the dash) so an empty electricity-price input doesn't show an `Annual (estimated) cost: —` label. The matching cross-instance figure in the **Multi-Instance Scale-Out** panel is renamed from `Total Annual Cost:` to `Total (estimated) Annual Cost:` for the same reason.
