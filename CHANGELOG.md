...
- **Final GPU inventory reconciled after machine scaling** (`sizer/sizer.js`, `tests/index.html`) — automatically managed GPUs per machine now scale back to the smallest model-supported count that keeps final N−1 utilization below 90%, avoiding unnecessary GPU purchases after the Sizer adds machines.
- **Minimum-fit procurement caveat added** (`sizer/sizer.js`, `tests/index.html`) — the first Sizing Note now presents an amber "Advisory" label within the bold "Advisory - minimum-fit hardware" heading for recommendations below 40 physical cores or 512 GB memory per machine, identifies them as minimum-fit results for the entered demand and growth setting, and prompts users to consider future workloads, model throughput, expansion, and OEM-validated configurations before purchasing hardware.
...
- **Validation** — all **1,549 / 1,549** browser tests pass, including exact Learn support matrices, workload requirement corrections, included-infrastructure guidance, deployment-topology parity, cluster-wide GPU-P partitioning, N−1 model-dependent total limits and GPU-driven machine recommendations, default-growth reconciliation, final GPU reconciliation, minimum-fit guidance, Sizing Notes, GPU metadata and S2D calculation handoff, report presentation contracts, Infrastructure IP Pool calculation, aggregate capacity, CPU catalog options, and schema drift.
