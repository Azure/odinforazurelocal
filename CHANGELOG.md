...
- **Release-candidate UI hardening** (`index.html`, `css/style.css`, `js/disaggregated.js`, `js/notifications.js`, `js/script.js`, `scripts/run-tests.js`, `sizer/sizer.css`, `tests/index.html`) — full Sizer and Designer matrices fixed phone overflow, native-dialog Escape dismissal, stale resume and architecture-dependent node state, and dynamically rendered option-card disabled semantics. Designer cards now expose aligned button, pressed, disabled, and tab-order state; confirmed disaggregated VRF choices use the same disabled contract; and the Rack-Aware selector keeps its documentation link as a separate keyboard control.
- **Validation** — all **1,575 / 1,575** browser tests pass; JavaScript, HTML, and CSS lint complete with zero errors; PR #292 passed CodeQL, dependency audit, test-result upload, and PowerPoint export smoke checks with the upgraded actions; and complete real-UI Sizer and Designer matrices cover supported scenarios, transitions, imports, outputs, handoffs, resets, themes, responsive layouts, and accessibility.
...
- **Sizer import buttons and onboarding restored ([#287](https://github.com/Azure/odinforazurelocal/issues/287))** (`sizer/sizer.js`, `s2d-calc/s2d-calc.js`) — scopes Sizer and embedded S2D onboarding cleanup to their own walkthroughs so first-time visitors can open the Import, Azure Migrate, and RVTools dialogs; Escape dismisses the visible dialog through its owning close path; and the Sizer tour now introduces both estate-import paths with an even feature-tile layout.
- **Validation** — all **1,572 / 1,572** browser tests pass; JavaScript, HTML, and CSS lint complete with zero errors; and the PowerPoint export smoke test succeeds.
...
- **Total VM GPU requirements expanded** (`sizer/`, `tests/index.html`) — Total VM mode uses a numeric DDA GPU input whose maximum follows the selected model's supported GPUs per machine and the 63-machine N−1 effective-capacity ceiling: up to 252 GPUs for four-per-machine models such as L40S and 126 GPUs for two-per-machine models such as A2. Per-VM and non-total workload inputs retain their model-specific per-machine limits, and changing models immediately clamps an oversized value.
- **Instance-wide GPU validation added** (`sizer/sizer.js`, `tests/index.html`) — save-time validation rejects same-model workload demand above the 63-machine N−1 effective-capacity ceiling while preserving the existing homogeneous-GPU conflict for mixed-model configurations. GPU model/count data now remains present in maximum-hardware recommendations so large GPU totals drive machine scaling instead of falling back to CPU-only recommendations.
- **Default growth reconciled at the GPU ceiling** (`sizer/sizer.js`, `tests/index.html`) — when the automatic 10% future-growth allowance alone would push a valid Total VM GPU requirement above N−1 capacity, Sizer resets that default to 0% and explains why. A manually selected growth allowance is preserved and produces an actionable GPU over-capacity warning instead.
...
- **GPU documentation link styling aligned** (`sizer/index.html`, `sizer/sizer.css`) — the hardware GPU guidance link now uses the same compact, theme-aware presentation as the S2D Calculator reference instead of the larger browser-default link style.
- **Manual GPU inventory shown in capacity results** (`sizer/sizer.js`, `tests/index.html`) — selecting physical GPUs per machine now displays the GPU capacity bar even without a GPU workload, showing zero consumed against the N−1 effective machine inventory.
- **Final GPU inventory reconciled after machine scaling** (`sizer/sizer.js`, `tests/index.html`) — automatically managed GPUs per machine now scale back to the smallest model-supported count that keeps final N−1 utilization below 90%, including zero after the final GPU workload is removed. Explicitly selected physical GPU inventory remains unchanged.
- **Minimum-fit procurement caveat added** (`sizer/sizer.js`, `tests/index.html`) — the first Sizing Note now presents an amber "Advisory" label within the bold "Advisory - minimum-fit hardware" heading for recommendations below 32 physical cores or 384 GB memory per machine, identifies them as minimum-fit results for the entered demand and growth setting, and prompts users to consider future workloads, model throughput, expansion, and OEM-validated configurations before purchasing hardware.
- **Single Node availability caveat added** (`sizer/sizer.js`, `tests/index.html`) — Single Node results now place a matching amber Advisory first in Sizing Notes, explaining that one machine provides no live-migration or workload-failover destination and that restart-requiring updates or host maintenance interrupt workloads.
- **Sizer scaling transitions stabilized** (`sizer/sizer.js`, `tests/index.html`) — Agentic Retrieval production defaults choose the first AKS-supported GPU that satisfies endpoint VRAM and synchronize Hardware Configuration; same-cycle topology and rack direction guards prevent recursive scale oscillation; final disaggregated rack capacity is rechecked; and resetting an ALDO Management configuration re-enables every workload card.
- **Integrated UI release validation added** (`.github/skills/`, `.github/copilot-instructions.md`, `docs/module-planning/`) — reusable Sizer and Designer skills exercise real rendered controls, state transitions, reset cleanup, synchronization, outputs, handoffs, responsive layouts, and accessibility in addition to static tests. Relevant PRs require the applicable smoke matrix, cross-tool PRs require both, and release candidates require both complete matrices. The PowerPoint export skill is migrated to the supported `.github/skills/pptx-export/SKILL.md` layout.
- **Designer UI and report reliability improved** (`js/script.js`, `report/`, `scripts/`, `tests/index.html`) — completed SAN designs now validate and explain their rack topology, storage type, ports, and confirmed adapter configuration instead of reporting four false HCI-field warnings and placeholder values. HTML, Markdown, Word, and PowerPoint preserve workflow metadata, all bounded Sizing Notes, and generic Advisory heading treatment consistently. The Example Templates picker is now a named keyboard-operable modal, and the welcome screen identifies AI workloads in Sizer coverage.
- **Sizing Notes joined to cluster design documents** (`sizer/sizer.js`, `js/script.js`, `report/`, `docs/json-schema/odin-design.schema.json`) — bounded plain-text Sizing Notes now travel with the Sizer-to-Designer payload and appear in both HTML and PowerPoint cluster design documents. Older and non-Sizer designs omit the section cleanly.
- **Design report fidelity improved** (`sizer/sizer.js`, `report/`, `docs/json-schema/odin-design.schema.json`, `tests/index.html`) — reports now use the title "Azure Local Instance | Design Configuration Report," identify Designer-only or Sizer-and-Designer workflow inputs with correct singular/plural wording, show Generated and Scenario metadata as compact inline label/value rows, describe designs as using an Azure connected or disconnected control plane with the disconnected connectivity mode, carry each workload's resolved GPU model and mode into HTML, Word, Markdown, and PowerPoint, match the Sizer's bold amber Advisory treatment, and label GPU racks as "GPU Enabled."
- **S2D volume guidance integrated with design reports** (`s2d-calc/`, `sizer/`, `report/`, `docs/json-schema/odin-design.schema.json`, `tests/index.html`) — Single Node, standard hyperconverged, and rack-aware Sizer results now show a compact maximum-volume calculation with the recommended volume count and a local S2D Calc link. The exact bounded result and derivation flow through Designer into HTML, Word, Markdown, and PowerPoint without recalculation, with HTML derivation bullets aligned inside their report card; ALDO management and disaggregated designs omit it.
- **GPU instance totals added to design reports** (`report/`) — Sizer-originated reports show the full NVIDIA model name per node and the total physical GPU count across the Azure Local instance.
- **ToR configuration planning linked from design reports** (`report/`, `scripts/smoke-test-pptx.js`) — Host Networking includes a visible planning aid linking to the portable ToR Switch Configuration Generator & Validator for example Cisco and Dell configurations. The Physical Network Configuration PowerPoint slide embeds the same clickable link, and both formats remind users to validate generated examples before deployment.
- **Infrastructure IP Pool minimum auto-filled** (`index.html`, `js/script.js`, `docs/json-schema/odin-design.schema.json`, `tests/index.html`) — entering a valid Starting IP in Designer Step 15 now calculates the minimum six-address Ending IP. A manually edited Ending IP is preserved, while an auto-generated value follows subsequent Starting IP changes, including after resume or import, and remains subject to existing CIDR and conflict validation.
- **Designer session and mobile navigation reliability improved** (`index.html`, `css/style.css`, `js/script.js`, `scripts/run-tests.js`, `tests/index.html`) — valid Infrastructure IP Pool edits are saved immediately for session resume, and compact navigation plus aligned breadcrumbs keep every utility control visible and pointer-accessible at 375 px widths.
- **AMD Turin catalog option added** (`sizer/sizer.js`, `tests/index.html`) — standard 5th Gen AMD EPYC processors now include the catalog-listed 84-core-per-socket option; Turin Dense remains unchanged with its 96-core minimum.
...
- **Foundry Local model discovery restored** (`sizer/sizer.js`, `scripts/run-tests.js`) — the Worker Profile input links directly to Microsoft's continuously updated Foundry Local model catalog so users can explore compatible OSS models without treating model choice as a fixed hardware-sizing preset.
...
- **Validation** — all **1,560 / 1,560** browser tests pass, including exact Learn support matrices, workload requirement corrections, included-infrastructure guidance, deployment-topology parity, cluster-wide GPU-P partitioning, N−1 model-dependent total limits and GPU-driven machine recommendations, default-growth reconciliation, zero-demand GPU reconciliation, compatible Agentic defaults, transition direction guards, Single Node and minimum-fit guidance, Sizing Notes, GPU metadata and S2D calculation handoff, architecture-aware report selections, template-dialog accessibility, Infrastructure IP Pool calculation and persistence, aggregate capacity, CPU catalog options, and schema drift. Real-UI stress testing also covered every deployment and workload type, all Designer scenario/architecture branches, mixed-workload scale-up/down, reset behavior, HCI/disaggregated transitions, report and ARM outputs, Sizer-to-Designer handoff, responsive dialogs, and Multi-Instance Scale-Out.
...
- **Agentic Retrieval sizing aligned with Microsoft Learn** (`sizer/sizer.js`, `tests/index.html`) — replaces the previous four-identical-worker approximation with three D8s_v3-equivalent CPU workers in every deployment, two embedding GPU workers for Combined and Knowledge modes, and no embedding workers for Agentic-only mode. The workload now exposes Combined, Knowledge, and Agentic deployment modes plus external, Foundry Local minimum, and Foundry Local production language-model endpoint choices. Local GPT-OSS-20B profiles add the documented dedicated CPU, memory, storage, GPU, and VRAM baseline. Agentic Retrieval accepts the full AKS Arc DDA list (T4, A2, A16, L4, L40, L40S, and RTX Pro 6000), presents NC8_A2 and NC8_A16 as recommended sizes, and rejects insufficient local model-host VRAM.
- **Agentic Retrieval payload and reports updated** (`docs/json-schema/odin-sizer.schema.json`, `report/`, `sizer/sizer.js`) — `SIZER_VERSION` is bumped from 2 to 3 for the new `deploymentMode` and `llmEndpoint` fields. Legacy payloads containing only `computeMode` import as Combined with an external endpoint, preserving their prior omission of local LLM capacity. Workload cards, Markdown/HTML reports, and PowerPoint output now use the Agentic Retrieval name and report the CPU-worker, embedding-GPU, and endpoint topology.
- **Dependency audit fixes applied** (`package.json`, `package-lock.json`, `scripts/check-npm-audit.js`) — raises `js-yaml` to at least 4.3.1 and both `brace-expansion` selector floors to patched 1.1.18 and 5.0.9 releases, removing their temporary exceptions. The exact dev-only nanoid advisory remains temporarily package-scoped because patched 3.3.17 is not available from the configured feed; the policy-aware audit continues to fail on any other unapproved high or critical advisory.
- **Validation** — all **1,504 / 1,504** browser tests pass, including deployment-mode, endpoint-capacity, AKS GPU-family, VRAM, legacy-import, schema-drift, and report contract coverage.
...
- **Product and maintainer documentation refreshed** (`README.md`, `CONTRIBUTING.md`, `docs/version-history/`, `.github/`, `scripts/run-tests.js`) — the README now describes the current Designer, Sizer, S2D, switch, import, report, privacy, and disconnected/offline workflows without duplicating the full release archive. Previous What's New summaries now live in a dedicated version-history document, while fail-fast regression checks enforce one current README release, correct prior-release rollover, and an accurate 30-entry repository map in the contribution guide. Maintainer guidance now follows shared `ODIN_VERSION` versioning, the `Release` to `main` workflow, and the report's from-scratch OOXML PowerPoint exporter. Full browser suite passes **1,501 / 1,501**.
- **Development dependency and vendor integrity hardening** (`package.json`, `scripts/`, `vendor/`) — available `ip-address`, `fast-uri`, and `postcss` security fixes and compatible validation-tool updates are installed, including `fast-uri` 4.1.2 for GHSA-7p8r-x3mc-p8w7. Exact dev-only `brace-expansion` advisories whose patched releases are unavailable remain visible as temporary package-scoped audit exceptions. Vendored JavaScript integrity checks now normalize line endings across operating systems, verify JSZip consistently, and enforce the documented official PptxGenJS 3.12.0 bundle hash.
- **S2D shared configuration validation tightened** (`s2d-calc/s2d-calc.js`) — shared URLs now reject out-of-range drive counts, invalid disk selectors and sizes, and oversized names before applying state. Minimum-machine guidance now uses clear grammar and capitalization. Existing valid generated URLs and calculations remain compatible.
...
- **Sizer import handling corrected** (`sizer/index.html`, `sizer/sizer.js`) — closing the shared import modal now clears both Azure Migrate and RVTools state without duplicate cleanup, the lazy ZIP reader reports a clear initialization failure before parsing, Azure Migrate inventories above the 1,000-machine limit report that specific limit instead of a generic incomplete-capacity message, and grouped workload storage rounds fractional averages up to avoid undersizing. The S2D Calculator hint now uses the established **Tip** wording.
- **Shared tool interactions corrected** (`sizer/sizer.js`, `switch-config/switch-config.js`) — the S2D Calculator hint remains available for every hyperconverged deployment type and hides only for disaggregated storage, while the ToR Switch navigation theme control now applies and persists the selected shared theme instead of raising a runtime error.
...
- **Storage Spaces Direct calculator added** (`s2d-calc/`, `js/nav.js`) — a new **S2D Calc** navigation destination provides browser-only maximum-volume and pool-consumption planning for Azure Local and Windows Server. The ODIN-styled page includes live validation, thin-extent and resiliency rules, single-tier and tiered pool estimates, formula derivation, reference examples, text-report export, named shareable configuration URLs, responsive layout, shared theming, source-retained MIT attribution, the same physical-machine terminology used by Designer and Sizer, a blue maximum-volume result treatment, and a first-visit walkthrough that can be replayed from the navigation Help button. Pool reporting distinguishes physical **Available pool capacity** from customer **Usable capacity (with X copies)** and excludes cache drives from tiered capacity. The volume count rounds usable capacity divided by the maximum supported individual volume size up, then uses the higher of that result or the machine count, ensuring equal Fixed volumes remain within the supported maximum while low-capacity configurations create at least one volume per machine. Azure Local automatically deducts the required 256 GB `Infrastructure_1` and 20 GB `ClusterPerformanceHistory` volumes from usable capacity; Windows Server does not. Sizer applies the same 276 GB platform-volume requirement throughout node recommendation, disk auto-scaling, utilization, and sizing notes, deducting it from hyperconverged S2D usable capacity or adding it to disaggregated SAN demand. Tiered layouts reject more cache drives than capacity drives, and pool formulas identify drive counts as applying to each server. Fixed provisioning also shows an equal logical size per volume; Thin provisioning instead explains that pool capacity is consumed as data is written and displays the current maximum supported individual volume size. A subtle advisory recommends three copies for increased resiliency and performance when two copies is selected with three or more machines. Single-tier pool planning starts at 8 drives per machine with 6.4 TB disks, while storage tiering starts at 2 cache and 4 capacity drives per machine with 6.4 TB capacity disks to preserve a 1:2 cache-to-capacity binding. After the user changes a calculator setting, the configuration is stored locally and the next visit offers **Resume** or **Start Fresh**; untouched defaults do not create a saved session. A new anonymous integer-only `s2dCalculation` counter appears as **S2D Calcs** in the shared seven-tile statistics banner, arranged 3+4 on desktop. It increments only for the first valid user-driven recalculation per page load, so exploring multiple settings does not inflate the total; deploying this release requires adding the matching increment-only child rule under `analytics/formCompletions` in Firebase Realtime Database.
- **S2D Help walkthrough refined** (`s2d-calc/s2d-calc.js`) — pool-estimation guidance now includes volume count and Fixed-versus-Thin planning, while the final result-sharing step uses a report-and-help icon.
- **S2D icon styling corrected** (`s2d-calc/s2d-calc.css`) — restored the missing SVG rule boundary so button and section icons retain their intended strokes without invalidating subsequent calculator styles.
- **Shared release branding and privacy footer** (`js/version.js`, `js/header-branding.js`, `js/footer.js`) — Designer, Sizer, ToR Switch, and S2D Calc now render their logo, release version, What's New control, and privacy disclosure from shared components. Branding asset bases reject scheme-based and protocol-relative URLs so shared logos remain local. `ODIN_VERSION` is the single application-release source and also stamps Designer exports; `SIZER_VERSION` remains the independent Sizer payload-format version.
...
- **Designer and Sizer interactions unified** (`js/notifications.js`, `css/interactions.css`) — shared theme-aware toasts now include accessible status semantics and dismiss controls. Reset, Start Over, Share URL, and Designer Export use consistent centered dialogs instead of browser-native prompts and confirmations, with an in-app fallback for Safari 14.
...
- **Knowledge tab sub-pages are now deep-linkable & shareable** (`index.html`, `js/script.js`) — opening the Knowledge tab and switching between its sub-pages (*Microsoft Sovereign Private Clouds reference architectures*, *Outbound Connectivity*, *AzLoFlows*) now reflects the current view in the address bar as `?tab=knowledge&topic=<slug>`, so the link can be copied and shared. Previously the `?tab=` parameter was stripped from the URL immediately after load and sub-page switches never touched the URL at all, so the address bar always showed the bare root URL with no way to link to a specific Knowledge page. Opening a shared link such as `?tab=knowledge&topic=outbound-connectivity` now restores both the Knowledge tab and the correct sub-page. Each sidebar nav item carries a stable `data-topic` slug (`reference-architectures`, `outbound-connectivity`, `azloflows`); the Designer (default) tab still resolves to a clean root URL, and the existing `?from=sizer` Sizer→Designer import flow is unaffected (only `tab` / `topic` params are rewritten).
...
- **4 PB storage-limit messages now point to Disaggregated Storage, and clarify what is actually gated** (`sizer/sizer.js`) — when a Hyperconverged design's total raw cluster storage exceeds the 4 PB (4,000 TB) per-storage-pool maximum, both the storage-limit banner and the matching sizing note now suggest switching the deployment type to **Disaggregated Storage** (external SAN — removes the internal-storage limit) alongside the existing "reduce machines, disk count, or disk size" advice. The banner previously read "export is blocked until corrected", but only the Word export was actually gated (PDF / CSV / JSON all worked) — the real gate is **Configure in Designer**. The banner now reads ““Configure in Designer” is blocked until corrected (reports can still be exported)”, the now-misleading Word-export block was removed so all report exports behave consistently, and the Configure-in-Designer alert also points to the Disaggregated option. The **Configure in Designer** button is now also greyed out (disabled) with an explanatory hover tooltip while the limit is exceeded — matching the existing disabled-state behaviour for >90% utilisation and over-capacity VMs — so the blocked action is visible up-front instead of only on click.
- **GitHub Enterprise Local promoted to Public Preview in the Knowledge tab** (`docs/reference-architectures/script.js`) — the GHEL reference architecture card and its *About this architecture* narrative now read **Public Preview** (was *Private Preview*), matching the Sizer's GHEL workload modal. The *About this architecture* text also now carries a **Learn more** link to [What is GitHub Enterprise Local? (preview)](https://aka.ms/GHEL). For badge consistency, the **Foundry Local** card now also reads **Public Preview** (was a *New* badge), its *About this architecture* narrative carries a **Learn more** link to [What is Foundry Local on Azure Local?](https://learn.microsoft.com/en-us/azure/azure-sovereign-clouds/private/foundry-local/what-is-foundry-local-on-azure-local), and the **General Purpose Workloads** tile description now opens with “Azure Local VMs enable data residency, …”.
...
- **Two JSON Schemas (draft-07)** under `docs/json-schema/` — [`odin-design.schema.json`](docs/json-schema/odin-design.schema.json) (Designer `{ version, exportedAt, state }` envelope + `state`) and [`odin-sizer.schema.json`](docs/json-schema/odin-sizer.schema.json) (Sizer `{ _meta, data }` envelope + `data`, with a `workloads[]` model discriminated by workload `type`). Permissive/forward-compatible, with rich `title`/`description` so each schema doubles as documentation. Reverse-engineered drafts were staged in `docs/module-planning/` before promotion to these published paths.
