...
- **GitHub Enterprise Local promoted to Public Preview in the Knowledge tab** (`docs/reference-architectures/script.js`) — the GHEL reference architecture card and its *About this architecture* narrative now read **Public Preview** (was *Private Preview*), matching the Sizer's GHEL workload modal. The *About this architecture* text also now carries a **Learn more** link to [What is GitHub Enterprise Local? (preview)](https://aka.ms/GHEL).
