...
- **Designer UI and report reliability improved** (`js/script.js`, `report/report.js`, `scripts/run-tests.js`, `tests/index.html`) — completed SAN designs now validate and explain their rack topology, storage type, ports, and confirmed adapter configuration instead of reporting four false HCI-field warnings and placeholder values across HTML, Markdown, and Word output. The Example Templates picker is now a named modal with a labeled close button, keyboard-operable choices, Escape and focus-trap behavior, and focus restoration to its trigger.
