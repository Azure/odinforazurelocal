...
- **S2D shared configuration validation tightened** (`s2d-calc/s2d-calc.js`) — shared URLs now reject out-of-range drive counts, invalid disk selectors and sizes, and oversized names before applying state. Existing valid generated URLs and calculations remain compatible.
...
- **Sizer import handling corrected** (`sizer/index.html`, `sizer/sizer.js`) — closing the shared import modal now clears both Azure Migrate and RVTools state, Azure Migrate inventories above the 1,000-machine limit report that specific limit instead of a generic incomplete-capacity message, and grouped workload storage rounds fractional averages up to avoid undersizing. The S2D Calculator hint now uses the established **Tip** wording.
