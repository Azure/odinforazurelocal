...
- **Disaggregated report validation corrected** (`report/report.js`, `scripts/run-tests.js`) — completed SAN designs now validate and explain their rack topology, storage type, ports, and confirmed adapter configuration instead of reporting four false HCI-field warnings and placeholder values. The same architecture-aware values flow into HTML, Markdown, and Word output.
- **Example Templates dialog made accessible** (`js/script.js`, `tests/index.html`) — the Designer template picker is now a named modal with a labeled close button, keyboard-operable template choices, Escape and focus-trap behavior, and focus restoration to its trigger.
