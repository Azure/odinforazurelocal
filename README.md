...
## Version 0.22.63 - Available here: https://aka.ms/ODIN
...
<a id="whats-new"></a>
### 🎉 Version 0.22.63 - Latest Release

> **Splits Multi-Instance Scale-Out into its own metric pair — Total Annual Energy (kWh/yr) plus an optional, currency-agnostic Electricity Price → Total Annual Cost input independent of the per-instance one — and now persists the Number of Instances + both Electricity Price boxes across page reloads, Resume, 🔄 Reset, Share Sizer Config as URL, and JSON import/export.** Also completes the **Sizer terminology refresh** from "Node" → "Physical Machine" for physical-server references (failover-cluster terminology like *Single Node*, *1 Node*, *N Nodes*, *cluster node* is unchanged), refreshes the **first-visit Help tour** for current capabilities, defaults **Allow for Future Growth** to **10 %** with an AUTO/MANUAL badge, tightens the dismiss-X behaviour on the **Number of Physical Machines** badge so it cascades cleanly to the resiliency dropdown, aligns **Storage Resiliency** dropdown wording (50 % / 33 % efficiency), and adds the three new Multi-Instance fields to the published Sizer JSON Schema.

**What's new**
- **Multi-Instance Scale-Out — Total Annual Energy & optional Total Annual Cost** in the Sizer — the multi-instance summary now shows a **Total Annual Energy (kWh/yr)** figure derived from per-instance annual energy × number of instances, plus a new optional, currency-agnostic **Electricity Price (per kWh)** input that drives a **Total Annual Cost** value (`per-instance annualKwh × instances × price`). The multi-instance price sits on its own line under the energy figure with an "Input:" hint and an ⓘ tooltip, and is fully independent of the per-instance Electricity Price already in the *Estimated Power, Heat & Rack Space per Instance* panel — leave either, both, or neither populated.
- **Persistence for Multi-Instance Scale-Out controls + the per-instance Electricity Price** — the *Number of Instances* dropdown, the per-instance Electricity Price, and the multi-instance Electricity Price are now part of `getSizerState()` so they save to `localStorage` on every change and recalc, and are restored by both **Resume** (from the banner) and **Apply** (from JSON / URL import). That means **Share Sizer Config as URL** and **Import / Export JSON** also round-trip them. Previously they all reverted to defaults on reload.
- **Sizer terminology refresh — "Nodes" → "Physical Machines"** for physical-server references throughout the Sizer (user-facing labels, hardware-requirement summaries, sizing notes, power & rack-unit breakdowns, GPU lock tooltips, host-overhead notes, Word export, and the 3D rack legend / per-server tile labels). Aligns with published Azure Local product terminology. **Failover-cluster terminology stays as "Node"** wherever it refers to the cluster role rather than the hardware (e.g. the **Single Node** deployment type, the *1–16 Nodes* / *N Nodes per Rack* dropdown values, *cluster node*, *witness node*, *storage node*, *worker node*, *AKS Arc node pools*).
- **Sizer first-visit Help tour refreshed** — the three-step onboarding modal now reflects current Sizer capabilities: the *Workload Modelling* tile lists every supported workload (VM, AKS Arc, AVD, GHEL, SQL, Foundry, File Server); the *Deployment Types* tile lists **Single Node, Hyperconverged, Rack-Aware Cluster, Disaggregated Storage, or ALDO Management Cluster** (matching the actual dropdown order); *Auto-Save & Import/Export* mentions the **RVTools** estate import; the *Pro Tips* page documents the **10 % Future Growth default**, the **Multi-Instance Scale-Out** view, and the *Auto-Scaling* rule that the engine grows cores / memory / disks per machine before adding more machines. Onboarding key bumped so existing visitors see the refreshed tour once.
- **Sizer "Allow for Future Growth" now defaults to 10 % (recommended)** — first-time visitors see a realistic sizing envelope instead of *No additional growth*. The dropdown shows an **AUTO** badge on fresh load so it's clear the 10 % is a recommended default; any change — including switching back to *No additional growth* — replaces it with a **MANUAL** badge so an explicit "no growth" choice is preserved and visible. Dismissing the MANUAL badge with × reverts the dropdown back to the **AUTO 10 %** default. Restoring a previous Sizer session leaves the saved value untouched and shows no badge.
- **Storage Resiliency dropdown wording aligned** — *Two-way Mirror (50% efficiency)* / *Three-way Mirror (33% efficiency)* now match the existing *Four-way Mirror (25% efficiency)* (rack-aware) and *Three-way Mirror (33% efficiency)* (ALDO Management) labels in both the static 2-node default and the dynamic 2-node rebuild path. The previous mixed *(min N nodes)* / *(N% efficiency)* wording was inconsistent.
- **"Info: Sizer hardware scaling weighting logic" link moved to its own line** under the *Physical Machine(s) — Example Hardware Configuration* section description so it's easier to spot and no longer crowds the first sentence.
- **🔄 Reset now clears Multi-Instance Scale-Out controls** back to their defaults (Number of Instances → 1, per-instance Electricity Price → blank, multi-instance Electricity Price input → blank, cached multi-instance price → empty), matching the rest of `resetScenario()`'s clean-slate behaviour.
- **Three new top-level fields documented in the published Sizer JSON Schema** (`docs/json-schema/odin-sizer.schema.json`) — `instanceCount`, `powerPriceInput`, `multiPowerPriceInput`. Backwards-compatible (the fields are optional). The schema-drift CI guard picks them up automatically as part of `getSizerState()`.
- **Fix: Sizer Number of Physical Machines dismiss-× now cascades cleanly to resiliency** — clicking the dismiss-× on the **MANUAL** badge for *Number of Physical Machines* now resets the dropdown to the current cluster type's minimum (1 / 2 / 3) **and** re-runs the resiliency-options + cluster-info refresh, so the **Storage Resiliency** dropdown immediately reflects the new node count (e.g. a 3-way mirror that's no longer valid at 2 machines is removed) instead of leaving a stale combination on screen.
