...
- **AUTO-scaled field state now survives F5 / Resume** — the in-memory `_autoScaledFields` Set and `_disaggAutoUpgraded` one-shot guard were not persisted in the saved Sizer state, so after the user pressed **F5** and clicked **Resume**, every AUTO badge was lost (including the cluster-type badge required by the auto-downgrade). Symptom: removing a MANUAL override on **Memory** after Resume left the cluster on Disaggregated forever, instead of dropping back to Hyperconverged like it did in the same session before F5. The saved state now carries `autoScaledFields` (string array of field IDs) and `disaggAutoUpgraded` (boolean); Resume re-hydrates both before the next calculation, the AUTO badges reappear, and the symmetric auto-downgrade fires correctly. Schema updated; both fields are optional and backwards-compatible.
