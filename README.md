...
- **UX polish** — the optional electricity-price annual cost in the *Estimated Power, Heat & Rack Space per Instance* panel now shows a small grey `Annual (estimated) cost:` label (matching the `Input:` label above it) so the value reads clearly as e.g. `Annual (estimated) cost: 47,041/yr` instead of the bare `47,041/yr`. The label only appears when the price field is populated. The matching figure in the **Multi-Instance Scale-Out** panel is renamed from `Total Annual Cost:` to `Total (estimated) Annual Cost:` for consistency. Also added a small vertical gap between the `📈 Capacity Runway` collapsible and the `Azure Local Instance — Sizing Notes` block so the two cards no longer butt up against each other.
