...
> **Selecting Disaggregated Storage no longer sizes the internal disks to hold the workload's storage.** Previously the auto-scaler grew the internal capacity disks to absorb the full workload storage on every cluster type, so on Disaggregated Storage the greyed-out *Physical Machine Storage Configuration* (and the exported JSON) showed a proportional share of the external SAN as if it were local disk. On a disaggregated cluster the workload data lives on a SAN, so the physical machines only need OS / boot disks. The Sizer now resets the internal disks to a fixed **2 × 0.96 TB** boot mirror (`DISAGG_BOOT_DISK_COUNT` / `DISAGG_BOOT_DISK_SIZE_TB`), excludes internal storage utilisation from the node-count decision, and resets the disk fields when the deployment type switches from Hyperconverged to Disaggregated. When a Hyperconverged design's raw cluster storage exceeds the 4 PB (4,000 TB) per-pool maximum, the storage-limit banner and sizing note now point the user to switch to Disaggregated Storage (external SAN). Sizer-only; no JSON-schema shape change (the `capacityDiskCount` / `capacityDiskSize` fields already exist — only their values change).

#### 0.22.66 - iSCSI external SAN promoted from "Coming Soon" to Preview + supported-SAN reference

> **The Disaggregated Designer's DA1 storage-type step no longer gates iSCSI as "Coming Soon".** Microsoft published external storage array (SAN) attach for Azure Local (version 2604+), with **iSCSI in Public Preview**, so the two iSCSI cards now show a **Preview** badge (Fibre Channel is left unbadged), the "⚠️ Feature not available yet" notes are removed, and a new availability note documents the 2604+ requirement plus a collapsible **Supported SAN arrays** list (Dell PowerStore, Pure Storage FlashArray, Hitachi VSP One, HPE Alletra MP 10000, NetApp ONTAP, Lenovo ThinkSystem) linking to Microsoft Learn. The Sizer Storage Connectivity row gets the same note. Presentation/content only — the wizard logic and the Sizer already supported all three storage types, so there is no behaviour change and no JSON-schema change.
