...
- **Sizer "Allow for Future Growth" now defaults to 10 % (recommended)** — first-time visitors see a realistic sizing envelope instead of *No additional growth*. The dropdown shows an **AUTO** badge on fresh load so it's clear the 10 % is a recommended default rather than a hidden adjustment, and any user change to the field — including switching back to *No additional growth* — replaces the AUTO badge with a **MANUAL** badge so an explicit "no growth" choice is preserved and visible. Restoring a previous Sizer session leaves the saved value untouched and shows no badge.
- **Fix: Sizer "VM cannot be placed on a single machine" false positive for Total VM Requirements input** — the per-VM placement validator compared the workload's `vcpus` / `memory` fields against per-node capacity, but in **Total VM Requirements** input mode those fields hold the fleet-wide totals (with `count = 1`), not the spec of any individual VM. A modest 1000-VM fleet totalling 4000 vCPUs / 16 000 GB was therefore flagged with a red "🚫 This VM cannot be placed on a single machine" note, fired the *"One or more VMs exceed per-machine capacity"* toast, and blocked the **Configure in Designer** button. The check now runs only for `inputMode === 'per-vm'` workloads, where per-VM semantics actually hold.
- **Fix: Sizer 3D Hardware Visualization — single-rack default zoom now consistent regardless of node count.** Previously the single-rack camera distance scaled by node count, so the empty default 2-node page looked noticeably smaller / further away from the rack than the post-calculation view of a busy 8–16-node deployment. The single-rack path now uses one fixed camera distance so the full 42U rack body (plus the core switch above) fills the frame identically on first load and after any calculation. Pure camera-distance tweak; look-at target, tilt, camera angle, and the multi-rack / rack-aware / disaggregated / two-row layouts are all unchanged.
