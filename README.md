...
- **Sizer Estimated Power now shows Annual Energy (kWh/yr) and an optional running cost** — the *Estimated Power, Heat & Rack Space* panel adds an estimated yearly energy figure (total Watts run 24/7/365) plus an optional **Electricity Price (USD/kWh)** box that estimates the annual running cost (with an ⓘ hover tip and a currency-converter link), alongside the existing per-machine Watts, total Watts, and BTU/hr.
