...
- **UX polish** — the optional electricity-price annual cost in the *Estimated Power, Heat & Rack Space per Instance* panel now shows a small grey `Cost:` label (matching the `Input:` label above it) so the value reads clearly as e.g. `Cost: 47,041/yr` instead of the bare `47,041/yr`. The `Cost:` label only appears when the price field is populated. Also added a small vertical gap between the `📈 Capacity Runway` collapsible and the `Azure Local Instance — Sizing Notes` block so the two cards no longer butt up against each other.
