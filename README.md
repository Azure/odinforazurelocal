...
- **Designer UI and report reliability** — completed SAN designs now report architecture-aware topology, storage, ports, and adapter configuration without HCI-only false warnings, while the Example Configuration Templates picker is a named keyboard-operable modal with Escape handling, focus containment, and focus restoration.
