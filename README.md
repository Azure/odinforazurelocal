...
- **Switch Config — Per-Rack Leaf Switches (up to 8 racks / 16 ToRs)**: New "Per-Rack Leaf Switches" panel (SC1b) for disaggregated deployments with 2–8 racks. Each rack gets its own collapsible card with editable ToR-A / ToR-B / BMC hostnames, per-rack leaf BGP ASN (auto-increments from 64789), loopback pair, Mgmt SVI IPs, and site. The generator emits up to 16 ToR configs + 8 BMC configs in one run, with labels `Rack N — ToR-A (hostname)`. Output tabs scroll horizontally when they exceed the container width
