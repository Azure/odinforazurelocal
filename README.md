...
- **Hardware Sizer**: Workload-based hardware sizing for VMs, AKS, and AVD — calculates CPU, memory, storage, GPU, and node requirements with auto-scaling and power/rack-space estimates, and a 3D visualization of hardware in one to four 42U racks. Import existing estates directly from a VMware **RVTools** `.xlsx` export (parsed entirely in-browser) to seed workloads
...
> **Capacity Runway projection rewritten — accuracy fixes + new "Size hardware for 5 years of compound YoY growth" No/Yes dropdown (issue #254).** The old projection had several bugs (wrong storage element ID, wrong TB↔GB conversion, round-trip drift making Year 1 read ≥ 100 %, no GPU column, no disaggregated-SAN path); these are all fixed. Above the projection table there is now a new opt-in **No/Yes dropdown** that **compounds the existing growth percentage over the full 5-year window** — so 10 % YoY becomes ×1.61, and `getGrowthFactor()` automatically routes that through the rest of the Sizer (workload totals, node recommendation, auto-scale loop, capacity bars, sizing notes). Set it to **Yes** and the hardware is sized to fit Year 5 demand instead of Year 1, and the runway table's bottom hint flips to confirm "all five years should fit within capacity".

**What's new**
- **New No/Yes dropdown: *Size hardware to accommodate 5 years of compound YoY growth*** (issue #254) inside the `📈 Capacity Runway` section in the Sizer. Defaults to **No** — preserves the historical "Year 1 + growth %" sizing. Switch to **Yes**, and `getGrowthFactor()` switches from `(1 + pct/100)` to `(1 + pct/100)^5` (10 % → ×1.61, 20 % → ×2.49, 30 % → ×3.71, 50 % → ×7.59). Because every Sizer calculation already routes through `getGrowthFactor()`, this single switch propagates automatically into workload totals, the node-count recommendation, the auto-scale / rack-shrink loops, the capacity bars, the GPU bar, and the sizing notes — no per-call-site changes. State persists through localStorage save/resume, JSON export/import, Share-as-URL, and 🔄 Reset (where it returns to **No**).
...
- **Disaggregated → Hyperconverged auto-downgrade** — symmetric inverse of the existing auto-upgrade. When the cluster was auto-upgraded to Disaggregated (the **Deployment Type** dropdown still carries its **AUTO** badge), and workloads are later removed so the design now fits in **≤ 12 machines on standard Hyperconverged** (16 × 0.80 headroom), the Sizer now drops `Deployment Type` back to **Hyperconverged** in a single step (skipping the rack-by-rack shrink) and shows a toast. A 4-machine gap between the upgrade threshold (recommendation > 16) and the downgrade threshold (recommendation ≤ 12) gives a stable [13..16] band so workloads near the boundary do not ping-pong between the two deployment types. A user who manually changed the **Deployment Type** is never disturbed.
- **AUTO badge on Deployment Type now persists across the post-upgrade recalc** — the auto-upgrade fires once and re-runs the calculation; on that second pass the upgrade branch can't re-fire (we're already disaggregated) so the badge was being wiped by `clearAutoScaledHighlights()`. That also broke the new auto-downgrade, which needs the badge to know it's allowed to drop back to HCI — so e.g. reducing memory from 2 TB to 1 TB per node would auto-upgrade to Disaggregated as expected, but then removing the MANUAL override on memory would land at 2 racks of 7 nodes instead of going back to Hyperconverged. The badge now persists across the recalc (same re-apply pattern already used for `disagg-rack-count` and `future-growth`), so AUTO stays visible on **Deployment Type** and the symmetric downgrade fires correctly when the trigger condition disappears.
- **AUTO-scaled field state now survives F5 / Resume, JSON Import, and Share-via-URL** — the in-memory `_autoScaledFields` Set and `_disaggAutoUpgraded` one-shot guard were not persisted in the saved Sizer state or in exported JSON, so after the user pressed **F5** and clicked **Resume** — or after opening a Shared URL, or after importing an exported JSON — every AUTO badge was lost (including the cluster-type badge required by the auto-downgrade). Symptom: removing a MANUAL override on **Memory** after Resume / Import / URL-load left the cluster on Disaggregated forever, instead of dropping back to Hyperconverged like it did in the same session before the reload. The saved state and exported JSON now carry `autoScaledFields` (string array of field IDs) and `disaggAutoUpgraded` (boolean); both Resume and Import re-hydrate them before the next calculation, the AUTO badges reappear, and the symmetric auto-downgrade fires correctly. Schema updated; both fields are optional and backwards-compatible, so older JSON exports continue to import cleanly.
- **UX polish** — the optional electricity-price annual cost in the *Estimated Power, Heat & Rack Space per Instance* panel now shows a small grey two-line `Annual cost:` / `(estimated)` label (matching the `Input:` label above it) so the value reads clearly as e.g. `Annual cost: 47,041/yr` instead of the bare `47,041/yr`. The label only appears when the price field is populated. The matching figure in the **Multi-Instance Scale-Out** panel is renamed to `Total Annual Cost:` / `(estimated)` (also two-line) for consistency and to stop the cell wrapping awkwardly. The **Total Annual Energy** figure (both single-instance and multi-instance) now auto-scales from `kWh/yr` to `MWh/yr` at ≥ 1 M kWh, so a 10× scale-out reads `1,605.7 MWh/yr` instead of `1,605,710 kWh/yr`. Also added a small vertical gap between the `📈 Capacity Runway` collapsible and the `Azure Local Instance — Sizing Notes` block so the two cards no longer butt up against each other. The Capacity Runway section header itself is also renamed to `📈 Capacity Runway — 5 Year-over-Year (YoY) Additional Workload Growth Projection` so the 5-year horizon and the *additional* nature of the growth (on top of today's workloads) are obvious from the header alone, with the *Allow for Future Growth* tooltip updated to match.
- **Sizer polish — final pass for this release**:
  - **Growth-applied banner** under the four Total Requirements cards. When *Future Growth* > 0 % or the new 5-year compound switch is **Yes**, a small yellow tip appears explaining where the multiplier came from — e.g. *"Totals above include 20 % YoY growth compounded over 5 years (×2.49) — hardware is sized for the full 5-year horizon."* — so the difference between raw workload demand and the hardware-sizing target is no longer invisible. Hidden when both growth = 0 % and years = 1.
  - **Disaggregated auto-rack-scale toast wording** now reads *"exceeds N racks capacity"* instead of *"exceeds (N × 16)-machine capacity"*, and reports the **final** rack count after the recursive recalc settles (a 6 → 7 → 8 cascade only shows the innermost toast).
  - **Auto rack-scale now climbs all the way to 8 racks** (was hard-capped at 4). The cap was redundant because the caller already respects a MANUAL pin on Number of Racks. **Crucially, the bump-walk skips rack counts that don't add capacity** — per-rack cap is non-monotonic (1–4 racks → 16/rack = 64 total; 5 → 60; 6 → 60; 7 → 63; 8 → 64), so from 5 racks (60) auto jumps straight to 7 (63) skipping the no-op 5 → 6 step. From 4 racks (already at the 64-machine architectural max) auto correctly does nothing — no later N has a higher cap.
  - **MANUAL-override capacity warning** on Disaggregated now suggests *increasing Number of Racks* when it would genuinely help (over-utilised resource is Compute or Memory, Number of Racks is pinned at MANUAL, the cluster is at its per-rack cap, and a higher rack count exists whose total capacity is strictly greater than current). Stays silent when the only over-utilised resource is Storage (SAN-backed, more racks don't help) or when no later N would beat the current total cap.
  - **Multi-Instance Scale-Out — SAN-power note on Disaggregated** — the multi-instance summary panel now shows the same *"SAN storage appliance/array power is not included — consult your SAN vendor"* callout that already appears on the per-instance power panel, but only when the deployment type is `Disaggregated Storage`, so the omission isn't lost when reading the cross-instance roll-up.
...
#### 0.22.65 - Capacity Runway rewrite + No/Yes dropdown to size for 5-year compound YoY growth (issue #254)

> **Capacity Runway projection rewritten — accuracy fixes plus a new opt-in No/Yes dropdown that sizes the hardware for the full 5 years of compound YoY growth** (issue #254) instead of just Year 1. Because every Sizer calculation already routes through `getGrowthFactor()`, the single switch from `(1 + pct/100)` to `(1 + pct/100)^5` automatically propagates into workload totals, the node-count recommendation, the auto-scale / rack-shrink loops, the capacity bars, the GPU bar, and the sizing notes — no per-call-site changes.

- **New No/Yes dropdown: *Size hardware to accommodate 5 years of compound YoY growth*** (issue #254) inside the `📈 Capacity Runway` section. Defaults to **No**. Set to **Yes** and `getGrowthFactor()` switches from `(1 + pct/100)^1` to `(1 + pct/100)^5` (10 % → ×1.61051, 20 % → ×2.48832, 30 % → ×3.71293, 50 % → ×7.59375). Persists through `localStorage` save/resume, JSON export/import, Share-as-URL, and 🔄 Reset (returns to unticked). New `getGrowthYears()` helper + `onSizeFor5YrGrowthChange()` handler + new top-level `sizeFor5YrGrowth` (boolean) field in the published Sizer JSON Schema (backwards-compatible).
...
> **Disaggregated Storage Sizer auto-scales the rack count both ways** — up when the conservative node loop has hit the per-rack machine cap with utilisation still ≥ 90 % (preferring more racks / machines over escalating to 3-4 TB DIMMs), and back down when workloads shrink or are removed and the workload comfortably fits at fewer racks — and renames the per-rack dropdown label to **Physical Machines per Rack**.

- **Disaggregated rack auto-scale (up)** — when the cluster type is *Disaggregated Storage*, the user has **not** manually pinned *Number of Racks*, and the conservative auto-scale loop has reached the per-rack machine cap (`rackCount × 16`) with utilisation still ≥ 90 % on any resource (CPU / memory / storage / GPU), the Sizer now bumps *Number of Racks* up (1 → 2 → 3 → 4, capped at 4 racks) **before** falling through to aggressive memory / ratio escalation. This is the "prefer more racks over higher DIMMs" weighting — adding a rack of cheap 2 TB machines is preferred over jumping to 3-4 TB DIMMs at the same machine count. The dropdown gets a persistent **AUTO** badge, *Number of Physical Machines* is rebuilt for the new rack count, and the Sizer re-runs the calculation. **Going beyond 4 racks (5–8) stays a manual decision** because that forces fewer machines-per-rack (5 → 12, 6 → 10, 7 → 9, 8 → 8) — a real fabric / floor-space change. New `shouldAutoScaleDisaggRacks(currentRackCount, recommendedNodes, conservativeFailed)` helper covered by 16 unit tests.
- **Symmetric rack auto-shrink (down)** — when workloads are removed or shrunk and the rack count was previously AUTO-set (user still hasn't pinned it), the Sizer now drops one rack tier per recalc if the workload fits at (`currentRacks - 1`) racks with **≥ 20 % headroom**. Single-step + 20 % buffer prevents oscillation at the boundary. Manually-chosen rack counts are never disturbed. New `shouldAutoShrinkDisaggRacks(currentRackCount, recommendedNodes)` helper covered by 12 unit tests.
- **Dropdown label rename: *Nodes per Rack* → *Physical Machines per Rack*** in the Sizer's Disaggregated Storage view, matching the wider v0.22.63 "Nodes → Physical Machines" terminology refresh. The dropdown *values* themselves (e.g. "16 Nodes per Rack (32 total)") are intentionally **unchanged** because failover-cluster terminology still uses "Node".

#### 0.22.63 - Multi-Instance Annual Energy/Cost, persistence + resume fixes, terminology refresh, Help tour refresh

> **Splits Multi-Instance Scale-Out into its own metric pair — Total Annual Energy (kWh/yr) plus an optional, currency-agnostic Electricity Price → Total Annual Cost input independent of the per-instance one — and persists the Number of Instances + both Electricity Price boxes across page reloads, Resume, 🔄 Reset, Share Sizer Config as URL, and JSON import/export.** Also completes the **Sizer terminology refresh** from "Node" → "Physical Machine" for physical-server references, refreshes the **first-visit Help tour**, defaults **Allow for Future Growth** to **10 %** with an AUTO/MANUAL badge, aligns **Storage Resiliency** wording (50 % / 33 % efficiency), and adds three new Multi-Instance fields to the published Sizer JSON Schema.

- **Multi-Instance Scale-Out — Total Annual Energy & optional Total Annual Cost** in the Sizer — multi-instance summary shows a **Total Annual Energy (kWh/yr)** figure plus a new optional, currency-agnostic **Electricity Price (per kWh)** input that drives a **Total Annual Cost** value, independent of the per-instance Electricity Price.
- **Persistence for Multi-Instance Scale-Out controls + the per-instance Electricity Price** — *Number of Instances* and both Electricity Price boxes are now part of `getSizerState()` so they save to `localStorage`, round-trip through **Share Sizer Config as URL** and **Import / Export JSON**, and are restored by **Resume**.
- **Sizer terminology refresh — "Nodes" → "Physical Machines"** for physical-server references throughout the Sizer (UI labels, sizing summaries, power notes, Word export, 3D rack legend). Failover-cluster terminology stays as "Node" (Single Node, N Nodes per Rack dropdown values, cluster node, witness node, worker node, AKS Arc node pools).
- **Sizer first-visit Help tour refreshed** for current capabilities (all 7 workload types, 5 deployment types matching the dropdown order, RVTools import, 10 % Future Growth default, Multi-Instance Scale-Out, the auto-scaler's prefer-more-machines rule). Onboarding key bumped so existing visitors see the refreshed tour once.
- **Sizer "Allow for Future Growth" now defaults to 10 % (recommended)** with an **AUTO** badge; any user change replaces it with a **MANUAL** badge. Dismissing the MANUAL badge with × reverts to the AUTO 10 % default.
- **Storage Resiliency dropdown wording aligned** — *Two-way Mirror (50% efficiency)* / *Three-way Mirror (33% efficiency)* now match the existing rack-aware and ALDO Management labels.
- **🔄 Reset now clears Multi-Instance Scale-Out controls** back to defaults (Number of Instances → 1, both Electricity Price boxes blank).
- **Three new top-level fields documented in the published Sizer JSON Schema** — `instanceCount`, `powerPriceInput`, `multiPowerPriceInput` (backwards-compatible, optional).
- **Fix: Sizer Number of Physical Machines dismiss-× now cascades cleanly to resiliency** — dismissing the MANUAL badge resets the dropdown to the cluster's minimum AND refreshes `updateResiliencyOptions()` + `updateClusterInfo()` so the **Storage Resiliency** dropdown reflects the new node count immediately.

#### 0.22.62 - GitHub Enterprise Local (GHEL) Sizer workload, hardware scaling weighting info popup, 2-node memory-density 3-node preference
...
- **`SIZER_VERSION` bumped 1 → 2** to reflect the new `tier` / `ha` / `replicas` fields on GHEL workloads in the Sizer export payload. The importer continues to accept v1 payloads (forward-compatible).
- **Foundry Local references refreshed for Build 2026** — updated the SLM / LLM model-class examples in the Sizer to the current Foundry Local catalog (Phi-4, Phi-4-reasoning, gpt-oss-20b, gpt-oss-120b (vLLM-only), Mistral-7B-v0.2, DeepSeek-R1 7b/14b, Qwen3, NVIDIA Nemotron, Whisper), clarified the **ONNX-GenAI (CPU or GPU) vs vLLM (GPU only)** engine choice, called out **multi-node inference** scheduling across cluster-wide GPU / CPU capacity, and refreshed the *Learn more* links block.
- **Privacy footer refreshed + analytics opt-out added** across all 5 pages. The footer now clearly states that user inputs / configurations / template data stay in the browser and discloses anonymous integer-only usage counters. A new **Page views opt-out** checkbox sits in the top navigation bar (right of the light/dark theme toggle) — preference persisted in `localStorage` (`odin-analytics-opt-out=1`) — and analytics also auto-disables when the browser sends `navigator.doNotTrack` or `navigator.globalPrivacyControl`.
