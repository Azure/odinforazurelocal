...
(function () {
    ...
    var SUBTITLE_COLOR = 'A1A1AA';
    var HEADING_COLOR  = 'E5E7EB';
    var BODY_COLOR     = 'D1D5DB';   // bullet text — slightly brighter than A1A1AA for readability
    var CARD_FILL      = '1A1A24';
    var CARD_LINE      = '2D2D3A';
    var ACCENT_BLUE    = '0078D4';
    var ACCENT_PURPLE  = '7B68EE';
    ...
    function makeShape(o) {
        var adj = (o.prst === 'roundRect' && o.adj != null)
        ...
        if (o.text != null) {
            var tp = o.textProps || {};
            var anchor = o.textAnchor || 'ctr';
            var algn = o.textAlign || 'ctr';
            var rPrAttrs = ['lang="en-US"', 'sz="' + (tp.sz || 1200) + '"',
                tp.bold ? 'b="1"' : '', tp.italic ? 'i="1"' : '']
                .filter(Boolean).join(' ');
            var tfill = '<a:solidFill><a:srgbClr val="' + (tp.color || '000000') + '"/></a:solidFill>';
            var latin = '<a:latin typeface="Segoe UI"/>';
            var paras = String(o.text).split('\n').map(function (line) {
    ...
    function makeTextBoxSp(id, name, x, y, cx, cy, text, runProps) {
        var rp = runProps || {};
        var rPrAttrs = ['lang="en-US"', 'sz="' + (rp.sz || 1800) + '"',
            rp.bold ? 'b="1"' : '', rp.italic ? 'i="1"' : '']
            .filter(Boolean).join(' ');
        var fill = '<a:solidFill><a:srgbClr val="' + (rp.color || '000000') + '"/></a:solidFill>';
        var latin = '<a:latin typeface="Segoe UI"/>';
        var algn = rp.align || 'l';
    ...
    function makeBulletsTxBox(id, name, x, y, cx, cy, bullets, sz) {
        ...
        function bulletParagraph(item) {
            ...
            var marL = lvl > 0 ? Math.min(lvl * 164592, 658368) : 0;
            var pPr = '<a:pPr lvl="0" marL="' + (marL + 228600) + '" indent="-228600">'
    ...
    function buildCoverSlide(meta) {
        ...
        var footer = 'Generated by ODIN  ·  ' + nowStamp().replace('_', ' ');

        // Big centered ODIN logo above the title (2.2" square, slide is 13.33" wide).
        var logoSize = 2.2;
        var logoX = (13.33 - logoSize) / 2;
        var shapes = [
    ...
    function buildTextSectionSlide(plan, content) {
        ...
        var bullets = content.bullets.length ? content.bullets : [{ text: '\u2014', lvl: 1 }];
        var chrome = buildSlideChrome({
            title: plan.title,
    ...
    function buildPictureSectionSlide(plan, content, raster) {
        ...
        var diagCardX = 0.4,  diagCardY = 1.55;
        var diagCardW = 6.0,  diagCardH = 5.5;
        var bulletsCardX = 6.55, bulletsCardY = 1.55;
        var bulletsCardW = 6.4,  bulletsCardH = 5.5;

        // Fit-contain the diagram inside the card with 0.18" padding.
        var pad = 0.18;
        var maxImgW = diagCardW - 2 * pad;
        var maxImgH = diagCardH - 2 * pad;
        ...
        var imgX = diagCardX + (diagCardW - imgW) / 2;
        var imgY = diagCardY + (diagCardH - imgH) / 2;
    ...
    function buildBulletsWithFooterSlide(plan, content, raster) {
        ...
        var bulletsCardX = 0.4, bulletsCardY = 1.55;
        var bulletsCardW = 12.55, bulletsCardH = 3.4;

        // Bottom diagram card: full-width, ~2.0" tall above the footer line.
        var diagCardX = 0.4, diagCardY = 5.05;
        var diagCardW = 12.55, diagCardH = 2.0;
        ...
        if (raster && raster.png) {
            ...
            var pad = 0.12;
            var maxW = diagCardW - 2 * pad;
            var maxH = diagCardH - 2 * pad;
            var aspect = raster.aspect || (maxW / maxH);
            var w = maxW, h = w / aspect;
            if (h > maxH) { h = maxH; w = h * aspect; }
            var x = diagCardX + (diagCardW - w) / 2;
            var y = diagCardY + (diagCardH - h) / 2;
    ...
    function buildBodySvgSlide(plan, raster) {
        ...
        var cardX = 0.4, cardY = 1.55;
        var cardW = 12.55, cardH = 5.5;
        var shapes = chrome.concat([
            makeCard(2, 'BodyCard',
        ...
        if (raster && raster.png) {
            ...
            var pad = 0.18;
            var maxW = cardW - 2 * pad;
            var maxH = cardH - 2 * pad;
            var aspect = raster.aspect || (maxW / maxH);
            var w = maxW, h = w / aspect;
            if (h > maxH) { h = maxH; w = h * aspect; }
            var x = cardX + (cardW - w) / 2;
            var y = cardY + (cardH - h) / 2;
    ...
    function buildPresentationRelsXml(slideCount) {
        ...
        var themeRid = slideCount + 2;
        var presPropsRid = slideCount + 3;
        var viewPropsRid = slideCount + 4;
    ...
    function downloadReportPptx(_themeName) {
        ...
        try {
            ...
            var logoPromise = fetchLogoBase64();
            var plan = [Promise.resolve(buildCoverSlide(meta))];
            ...
            Promise.all([Promise.all(plan), logoPromise]).then(function (parts) {
                var slides = parts[0].filter(Boolean);
                var logoPng = parts[1];
