...
(function() {
    ...
    const SECTION_PLAN = [
        ...
        {
            // Sizer-only slide: S2D volume guidance computed before Designer handoff.
            title: 'S2D Calculation | Supported Maximum Volume Size',
            match: [],
            customExtract: extractSizerS2dCalculation
        },
        {
    ...
    // is `[{ text, color?, bold?, linkRid? }]` (used by extractProxyConfiguration
    // for the inline reference URL).
    function makeBulletsTxBox(id, name, x, y, cx, cy, bullets, sz) {
        const size = sz || 1200;
        function runXml(t, run) {
            const props = run || {};
            const col = props.color || BODY_COLOR;
            const hlink = (props.linkRid != null)
                ? '<a:hlinkClick xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" r:id="rId' + props.linkRid + '"/>'
                : '';
            return '<a:r><a:rPr lang="en-US" sz="' + size + '" dirty="0"' + (props.bold ? ' b="1"' : '') + '>'
        ...
        function bulletParagraph(item) {
            ...
            const body = (Array.isArray(runs) && runs.length)
                ? runs.map(function(r) { return runXml(r.text, r); }).join('')
                : runXml(text);
    ...
    function getWorkflowSubtitle(state) {
        const hasSizerData = Boolean(state && (state.sizerHardware
            || (Array.isArray(state.sizerWorkloads) && state.sizerWorkloads.length > 0)));
        return hasSizerData
            ? 'Based on inputs from ODIN Sizer and Designer workflows'
            : 'Based on inputs from the ODIN Designer workflow';
    }

    function buildCoverSlide(meta, state) {
        let customer = '';
        ...
        const subtitle = getWorkflowSubtitle(state);
        ...
        const shapes = [
            ...
            makeTextBoxSp(3, 'Title',
                ...
                'Azure Local Instance | Design Configuration Report',
                { sz: 3000, bold: true, color: TITLE_COLOR, align: 'ctr' }),
            makeTextBoxSp(4, 'Subtitle',
                emu(0.6), emu(customer ? 4.05 : 4.2), emu(12.1), emu(0.65),
                subtitle,
                { sz: 1600, color: SUBTITLE_COLOR, align: 'ctr' }),
        ...
        if (customer) {
            shapes.push(makeTextBoxSp(6, 'Customer',
                emu(0.6), emu(4.7), emu(12.1), emu(0.5),
                customer,
                { sz: 1400, color: BODY_COLOR, align: 'ctr' }));
        }
    ...
    function buildSectionSlide(plan, blocks, isDisagg) {
        ...
        if (typeof plan.customExtract === 'function') {
            ...
            if (extracted.paginateBullets) {
                const pages = [];
                const allBullets = extracted.bullets || [];
                for (let offset = 0; offset < allBullets.length; offset += 24) {
                    pages.push(buildTextSectionSlide({
                        title: plan.title + (offset > 0 ? ' (continued)' : '')
                    }, {
                        bullets: allBullets.slice(offset, offset + 24),
                        sources: extracted.sources || [],
                        bulletSz: extracted.bulletSz || null,
                        links: extracted.links || []
                    }));
                }
                return Promise.resolve(pages.length ? pages : null);
            }
            const ec = {
    ...
    function extractSizerWorkloads() {
        ...
        for (let i = 0; i < s.sizerWorkloads.length; i++) {
            ...
            if (wl.gpuType) {
                ...
                bullets.push({ text: 'GPU Type: ' + gpuLabel + (gpuMode ? ' (' + gpuMode + ')' : ''), lvl: 2 });
            }
            bullets.push({
    ...
    function extractSizerSizingNotes() {
        ...
        const bullets = notes
            ...
            .map(function(note) {
                ...
                if (!advisory) return { text: note, lvl: 1 };
                return {
                    text: note,
                    lvl: 1,
                    runs: [
                        { text: 'Advisory', color: 'B45309', bold: true },
                        { text: advisory[2], bold: true },
                        { text: advisory[3] }
                    ]
                };
            });
        ...
        return {
            bullets: bullets,
            bulletSz: 900,
            paginateBullets: true,
            sources: ['Sizing Notes & Recommendations (from Sizer)']
        };
    }

    function extractSizerS2dCalculation() {
        const s = (typeof window.__odinGetReportState === 'function')
            ? window.__odinGetReportState() : null;
        const calc = s && s.sizerHardware && s.sizerHardware.s2dCalculation;
        if (!calc) return null;

        const bullets = [
            { text: 'Number of Volumes to Create: ' + calc.volumeCount, lvl: 1 },
            { text: 'Maximum supported size of each volume: ' + calc.maximumVolumeLabel, lvl: 1 },
            { text: 'Provisioning: ' + calc.provisioning + ' · Data Copies: ' + calc.copies + ' · Storage Layout: ' + calc.tiering, lvl: 1 },
            { text: 'How the result is calculated', lvl: 1 }
        ];
        (Array.isArray(calc.derivation) ? calc.derivation : []).slice(0, 10).forEach(function(line) {
            bullets.push({ text: String(line).slice(0, 500), lvl: 2 });
        });

        return {
            bullets: bullets,
            bulletSz: 850,
            sources: ['S2D Calculation (from Sizer)']
        };
    }
    ...
    function downloadReportPptx(_themeName) {
        ...
        try {
            ...
            const plan = [Promise.resolve(buildCoverSlide(meta, reportState))];
            ...
            Promise.all([Promise.all(plan), logoPromise]).then(function(parts) {
                const slides = [];
                parts[0].forEach(function(result) {
                    if (Array.isArray(result)) {
                        result.forEach(function(slide) { if (slide) slides.push(slide); });
                    } else if (result) {
                        slides.push(result);
                    }
                });
