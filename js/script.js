...
function selectOption(category, value) {
    ...
    if (category === 'scenario') {
        ...
        state.infra = null; state.infraCidr = null; state.infraCidrAuto = true; state.infraGateway = null; state.infraGatewayManual = false; state.infraPoolManual = false; state.infraPoolEndAuto = false; state.infraVlan = 'default'; state.infraVlanId = null;
        state.nodeSettings = [];
...
function updateUI() {
    ...
    if (infraInputStart && infraInputEnd) {
        ...
        if (state.infra) {
            ...
            if (infraInputEnd) {
                if (state.infraPoolEndAuto) {
                    infraInputEnd.dataset.autoMinimumEnd = 'true';
                } else {
                    delete infraInputEnd.dataset.autoMinimumEnd;
                }
            }
        }
...
const INFRA_POOL_MIN_IPS = 6;

function getMinimumInfraPoolEnd(startIp) {
    const octets = String(startIp || '').trim().split('.');
    if (octets.length !== 4 || octets.some(octet => !/^\d{1,3}$/.test(octet) || Number(octet) > 255)) {
        return null;
    }

    const start = octets.reduce((value, octet) => (value * 256) + Number(octet), 0);
    const end = start + INFRA_POOL_MIN_IPS - 1;
    if (end > 0xFFFFFFFF) return null;

    return [
        Math.floor(end / 0x1000000) % 256,
        Math.floor(end / 0x10000) % 256,
        Math.floor(end / 0x100) % 256,
        end % 256
    ].join('.');
}

function updateInfraPoolEndFromStart() {
    const startInput = document.getElementById('infra-ip-start');
    const endInput = document.getElementById('infra-ip-end');
    if (!startInput || !endInput) return;

    const endWasAutoGenerated = state.infraPoolEndAuto || endInput.dataset.autoMinimumEnd === 'true';
    if (!endInput.value.trim() || endWasAutoGenerated) {
        const minimumEnd = getMinimumInfraPoolEnd(startInput.value);
        endInput.value = minimumEnd || '';
        if (minimumEnd) {
            endInput.dataset.autoMinimumEnd = 'true';
            state.infraPoolEndAuto = true;
        } else {
            delete endInput.dataset.autoMinimumEnd;
            state.infraPoolEndAuto = false;
        }
    }
...
function markInfraPoolEndManual() {
    const endInput = document.getElementById('infra-ip-end');
    if (endInput) delete endInput.dataset.autoMinimumEnd;
    state.infraPoolEndAuto = false;
    markInfraPoolManual();
}
...
function updateInfraNetwork() {
    ...
    if (count < INFRA_POOL_MIN_IPS) {
        if (err) {
            err.innerText = `Range too small. Minimum ${INFRA_POOL_MIN_IPS} IPs required (Current: ${count}).`;
            err.classList.remove('hidden');
        }
        state.infra = null;
        return;
...
function deriveInfraNetworkFromNodeIps() {
    try {
        ...
        // Pool: minimum consecutive IPs starting at max(lastNodeHost, firstHost+9) + 1
        ...
        let poolEndL = poolStartL + INFRA_POOL_MIN_IPS - 1;
        ...
        if (includesGateway) {
            ...
            poolEndL = poolStartL + INFRA_POOL_MIN_IPS - 1;
        }

        // Cap pool to lastHost; exclude broadcast-1 if gateway sits there.
        const poolCap = gatewayShifted ? (lastHost - 1) : lastHost;
        if (poolEndL > poolCap) {
            warnings.push('Subnet ' + cidr + ' is too small to fit ' + INFRA_POOL_MIN_IPS + ' pool IPs after the last node. Pool not auto-filled — please widen the subnet or set the pool manually.');
...
const onboardingSteps = [
    {
        ...
        features: [
            { icon: '⚖️', title: 'Sizer', text: 'Calculate hardware requirements based on your workloads (AI workloads, VMs, AKS, AVD and more)' },
