        ...
        <div style="background: var(--card-bg); border: 1px solid var(--glass-border); border-radius: 16px; padding: 20px; max-width: 700px; width: 100%; max-height: 90vh; overflow-y: auto; box-sizing: border-box;">
            ...
            <div style="color: var(--text-primary); line-height: 1.8;">
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.1); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    ...
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Adds <strong>RVTools Excel import</strong> to the Sizer (<a href="https://github.com/Azure/odinforazurelocal/issues/230" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #230</a>) &mdash; turn a VMware <strong>RVTools</strong> <code>.xlsx</code> export into Sizer workloads in a few clicks, entirely <strong>client-side</strong>. A new <strong>RVTools</strong> import tab and a dedicated <strong>&#x1F4CA; RVTools</strong> toolbar button read the <code>vInfo</code> (and, when present, <code>vCluster</code> / <code>vHost</code>) sheets, summarise each detected source cluster (VM count, vCPU, memory, storage, hosts), and let you import <strong>one or more</strong> clusters into the single-cluster Sizer with a totals banner for the whole file. Selecting several source clusters <strong>consolidates</strong> their workloads into one cluster (grouped bands merge across clusters; per-VM lists concatenate), seeding a generic <em>Consolidated</em> cluster name; a <strong>select-all</strong> checkbox in the picker header ticks every cluster in one click. Two consolidation modes &mdash; <strong>Grouped</strong> (bands VMs by vCPU / RAM size class, e.g. <em>4 vCPU / 8 GB &times;12</em>) and <strong>Per-VM</strong> (one workload per VM). <strong>Privacy:</strong> individual VM names appear <strong>only</strong> in Per-VM mode; Grouped names never include a VM name, and no RVTools data ever leaves the browser. Choose <strong>Provisioned</strong> or <strong>In Use</strong> storage and optionally include powered-off VMs (templates are always excluded). A new editable <strong>Cluster Name</strong> field (validated to failover-cluster / NetBIOS rules) is seeded from the imported cluster and carries through <strong>Sizer &rarr; Designer &rarr; ARM parameters</strong>. Excel parsing uses <strong>SheetJS Community 0.20.3</strong>, vendored locally (no CDN) and <strong>lazy-loaded</strong> only when the RVTools tab is first opened, with its SHA-256 pinned and verified by the test harness. This release also <strong>fixes a node-count sizing bug</strong>: the Sizer's auto-scale node loops used a flat 32 GB host-memory overhead (and reserved no host CPU cores) while the capacity bars and the <em>&gt;90% invalid</em> banner use the S2D-aware reserve model (<a href="https://github.com/Azure/odinforazurelocal/issues/232" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #232</a>). On high-memory nodes (&#8776; &ge; 512 GB/node) the loops stopped adding nodes at ~89% while the banner showed &gt;90%, so the recommendation tripped its own warning (e.g. 1536 GB/node was recommended at 2 nodes / 95% memory); both loops now use the same reserve math and the case correctly recommends 3 nodes. Plus clearer Sizer labels (<em>Total</em> &rarr; <em>Required</em>, a <em>Number of Workloads</em> box, reworded capacity heading, and the per-node requirements heading now states the active <em>X:1</em> vCPU overcommit ratio). New 50-assertion RVTools suite and a node-overhead regression suite; ESLint clean; HTML validation clean; full suite passes <strong>1,314 / 1,314</strong>.</p>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    ...
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Improves the Sizer's <strong>physical-host compute reserve / overhead model</strong> (<a href="https://github.com/Azure/odinforazurelocal/issues/232" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #232</a>) and reorients the <strong>rack-layout diagrams</strong> to fill bottom-up with real node names (<a href="https://github.com/Azure/odinforazurelocal/issues/233" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #233</a>), <strong>removes the deprecated Low Capacity deployment type</strong> (<a href="https://github.com/Azure/odinforazurelocal/issues/234" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #234</a>), and <strong>refines the imported Azure Local instance experience</strong> in the Sizer (<a href="https://github.com/Azure/odinforazurelocal/issues/235" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #235</a>). <strong>S2D-aware host reserve</strong> &mdash; instead of a single flat percentage, the reserve is now sized by cluster type and storage shape: CPU is <code>max(ceil(20%), 2)</code> for ALDO management, <code>max(ceil(10%), 1)</code> for disaggregated, and <code>max(ceil(10%), 2)</code> for standard S2D physical cores per node; memory is base <strong>32 GB</strong> (S2D) or <strong>24 GB</strong> (disaggregated) + S2D cache-metadata <code>ceil(4 GB &times; cacheTB)</code> + <strong>64 GB</strong> when ALDO management is hosted, floored at <strong>8 %</strong> of host RAM. The reserve flows consistently through node-count recommendation, the AMD core-upgrade path, auto-scaling, the capacity bar, sizing notes, and the growth projection, and a new collapsible <em>Physical host compute overhead &mdash; assumptions &amp; math</em> section shows the memory / CPU math as tables with links to the Hyper-V, S2D, and Azure Local system-requirements docs. <strong>Bottom-up rack diagrams</strong> &mdash; all four generators in <code>report/rack-svg.js</code>, the 3D view in <code>sizer/rack3d.js</code>, and the wizard preview in <code>js/disaggregated.js</code> now stack servers from the bottom of the rack upward (switches stay at the top, FC switches at U1-U2), and real Designer node names surface in the diagram via new <code>getRackNodeLabel()</code> / <code>getRackTorLabel()</code> helpers (falling back to <code>Node N</code> / <code>ToR N</code>). <strong>Low Capacity removed</strong> (issue #234) &mdash; the deprecated deployment type is gone from the Designer and Sizer (Designer now offers 5 templates) and the cluster-type mapping helpers no longer reference it. <strong>Imported instance refinements</strong> (issue #235) &mdash; imported CPU / memory / disk count &amp; size / node count are locked to <strong>MANUAL</strong> so auto-scaling can't silently change them; the vCPU overcommit ratio is decoupled from the physical-CPU lock so it still escalates (4&rarr;5&rarr;6) on a fixed cluster; a new <strong>Disaggregated Storage</strong> import option greys out the S2D capacity-disk picker and drops disks from the equation; the import <em>Capacity per disk</em> dropdown now matches the Sizer's (adds 8 / 12 / 16 / 20 TB); and the <em>Parse &amp; Preview</em> button is hidden after a clean parse so only <em>Load Cluster Configuration</em> remains. <strong>Report security controls</strong> &mdash; the report's <em>Security Configuration</em> section now always lists all seven controls (Drift Control, WDAC, Credential Guard, SMB Signing, SMB Cluster Encryption, BitLocker Boot Volume, BitLocker Data Volumes) as Enabled / Disabled; <em>Recommended</em> shows all seven enabled and <em>Customized</em> reflects each per-control choice. ESLint clean; HTML validation clean; all <strong>1,240 / 1,240</strong> unit tests pass.</p>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    ...
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Adds the <strong>Microsoft 365 Local &mdash; Medium-Scale</strong> reference architecture to the <em>Microsoft Sovereign Private Clouds</em> page (Knowledge tab), and visually groups co-located single-node clusters into shared rack cards for both the new Medium variant and the existing Large variant. Inside each merged rack card, every server is wrapped in its own thin dashed <em>Cluster N</em> sub-frame so it's visually obvious that every server is an independent Azure Local single-node cluster (one server / one role / its own quorum + S2D pool). <strong>Medium-Scale layout</strong>: 2 single-node Azure Local clusters hosting Exchange mailbox servers (rendered as <em>one shared rack card</em> with both servers stacked inside, each inside its own Cluster sub-frame) + 1 three-node Azure Local cluster hosting Exchange Edge Transport, SharePoint, Skype for Business and SQL &mdash; 3 Azure Local clusters / 5 servers total, drawn as 2 cards. <strong>Large-Scale refinement</strong>: the 4 single-node Exchange mailbox clusters now share a single rack card (Servers 1-4), and the 2 single-node Edge Transport clusters share another (Servers 5-6); the 3-node SharePoint/Skype/SQL cluster keeps its own card. They remain independent Azure Local clusters with their own quorum / S2D pool / lifecycle — the merged card just reflects that operators typically co-locate them in the same physical rack. Active Directory, Firewall, Load Balancer and the internal management network router remain infrastructure components on the management / compute networks &mdash; not separate Azure Local clusters. The on-screen SVG diagram and the PowerPoint export pick up the new variant automatically via the existing <code>tpl.scaleVariants</code> code path. Storage is fixed at <strong>S2D (local)</strong> (same constraint as Small / Large). ESLint clean; HTML validation clean; full test suite passes.</p>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    ...
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Cosmetic fix on the <strong>Microsoft Sovereign Private Clouds</strong> reference architectures page (Knowledge tab). The <em>Azure Local Cluster (up to 128 nodes)</em> title no longer renders with a misleading <code>*</code> (Coming soon) suffix &mdash; 128 nodes is documented and supported. Removed the stray <code>comingSoon: true</code> flag from the <code>cluster-128</code> entry in <code>cardForScale()</code> in <code>docs/reference-architectures/script.js</code>; both the on-screen SVG diagram and the PowerPoint export render the title cleanly. The unrelated <em>logical-isolation</em> tenancy <code>Coming soon</code> badge was deliberately left untouched. ESLint clean; HTML validation clean; all <strong>1,246 / 1,246</strong> unit tests pass.</p>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.12</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 12, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Two bug fixes plus a small wizard quality-of-life feature. <strong>Designer &middot; Step 15 "&#x1FA84; Auto-fill from Node IPs" button</strong> &mdash; one click derives the Infrastructure CIDR, Default Gateway, and Infra IP Pool start/end from the Node IPs already entered on Step 12; gateway prefers <code>.1</code> (pivots to broadcast-1 if a node already uses <code>.1</code>); pool size is fixed at 6 IPs (Cluster IP + 3 ARB + 2 spare) placed past the highest node IP with &ge;10 IPs of growth headroom; any field already manually edited is silently kept and reported in a toast. Closes <a href="https://github.com/Azure/odinforazurelocal/issues/221" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #221</a>. <strong>Designer &middot; Step 15 live "Infrastructure Subnet Utilisation" diagram</strong> &mdash; horizontal bar diagram (Default Gateway / Infra IP Pool / Node IPs / Unused, with counts) now renders inline below the inputs as the user types; same SVG also added to the on-screen Configuration Report's IP, Infrastructure Network &amp; VLAN section. Single source of truth in new <code>js/subnet-utilization.js</code> shared by wizard, report, and PPT export. <strong>Disaggregated rack diagram &middot; single-rack overlap and clipping resolved across both code paths</strong> (the wizard preview in <code>js/disaggregated.js</code> and the configuration report in <code>report/rack-svg.js</code>). Both generators now compute a content-aware width floor from the title + brand badge, SAN/iSCSI fabric labels, and legend, centre the rack column inside the widened canvas, and span the SAN/iSCSI box pair across the full canvas width; the <code>max-width</code> clamp was also removed so the SVG fills its container. Closes <a href="https://github.com/Azure/odinforazurelocal/issues/223" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #223</a>. ESLint clean; HTML validation clean; all <strong>1,246 / 1,246</strong> unit tests pass.</p>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.11</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 12, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);"><strong>Sizer hardware-options alignment with the Azure Local Solutions Catalog.</strong> A new automated build-time gap check (<code>scripts/catalog-gap-check.js</code>) compares every Sizer dropdown against a captured catalog snapshot and writes <code>test-results/catalog-gap-report.txt</code> / <code>.json</code> after the browser test suite. The Sizer now offers <strong>NVIDIA A100</strong> (80 GB / 300 W / MIG up to 7 slices &mdash; partitions deliberately stop at <code>1/8</code>, no <code>1/16</code>) and <strong>NVIDIA A40</strong> (48 GB / 300 W / vGPU up to 16 partitions); <strong>xeon-5th</strong> extended to <strong>128 cores/socket</strong> (Catalog v2026-05 top SKUs e.g. Platinum 8593Q); <strong>xeon-6</strong> adds <strong>36-core and 80-core</strong> options; <strong>40-drive capacity nodes</strong> (single-tier and two-tier); new capacity drive sizes <strong>8 / 12 / 16 / 20 TB</strong>; new cache size <strong>12.8 TB</strong>. Memory ceiling intentionally held at 4 TB per node (gap report flags this as deliberate density steering, not an oversight). ESLint clean; HTML validation clean; all <strong>1,171 / 1,171</strong> unit tests pass. Closes <a href="https://github.com/Azure/odinforazurelocal/issues/226" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #226</a>.</p>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.10</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 12, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Polish release for the <strong>Microsoft Sovereign Private Clouds reference architectures</strong> page (Knowledge tab) and its PowerPoint export. <strong>Tenant header alignment unified</strong> &mdash; strict and logical isolation cards now use the same label-above + dashed-border + chip group layout, so workload areas align byte-for-byte across cards in the same row regardless of tenancy. <strong>SAN-using clusters grouped together</strong> &mdash; a stable sort moves SAN-using clusters (cluster-64, cluster-128, plus single-node / cluster-16 with <code>SAN</code> or <code>S2D + SAN</code>) to the left of each band so the SAN cylinder visually anchors only those cards. <strong>Microsoft 365 Local switched to the official Microsoft 365 SVG logo</strong> (public-domain, sourced from Wikimedia Commons), replacing the older Exchange/SharePoint/Skype trio &mdash; used by both the on-screen card and the PPT hero card. <strong>PPT overview slide</strong> now uses shorter scale labels (<code>Single Node</code>, <code>Rack-Aware (2-8)</code>, <code>16 nodes</code>, <code>64 nodes</code>, <code>128 nodes</code>, <code>M365 Small</code>, <code>M365 Large</code>) joined with " + ", inside a wider auto-shrinking pill so long combinations no longer overflow. <strong>PPT per-purpose slide</strong> gets a new layout &mdash; compact <code>TENANCY</code> + <code>STORAGE</code> tiles on the left, and a wide <code>SCALE</code> panel on the right that lists every selected scale with its bold title + description. <strong>"RECOMMENDED" badge removed</strong> from the connectivity step. ESLint clean; HTML validation clean; all 1,160 unit tests pass; PPTX smoke test produces a valid file.</p>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.09</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 12, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);"><strong>Bug fix.</strong> Resolves <a href="https://github.com/Azure/odinforazurelocal/issues/222" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #222</a> &mdash; the <strong>SAN Volume LUN ID</strong> inputs on Designer Step 16 (<em>Storage Pool Configuration</em>) were shown whenever <strong>InfraOnly</strong> was selected, regardless of architecture. They are only consumed by the <strong>Disaggregated</strong> (<code>create-cluster-san</code>) ARM template; for <strong>Hyperconverged</strong> deployments they could not be filled in correctly and misled users into thinking they had to enter SAN details before continuing the wizard. The visibility check in <code>syncStoragePoolUi()</code> is now gated on <code>state.architecture === 'disaggregated'</code> only &mdash; HCI users picking <em>Express</em>, <em>InfraOnly</em>, or <em>KeepStorage</em> see only the storage-pool choice cards and can proceed straight to the next step. No state-machine, validation, or ARM-generation changes: the bug was purely in the DOM visibility step. New unit test covers all four <code>(architecture &times; storagePoolConfiguration)</code> combinations to lock the behaviour. ESLint clean; all 1,160 unit tests pass.</p>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    ...
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);"><strong>Security maintenance release.</strong> Bumps the transitive <code>fast-uri</code> npm dependency to <code>&gt;= 3.1.2</code> via a new <code>overrides</code> entry in <code>package.json</code> to clear two high-severity Dependabot alerts &mdash; <strong>GHSA-q3j6-qgpj-74h6 / CVE-2026-6321</strong> (path traversal via percent-encoded dot segments) and <strong>GHSA-v39h-62p7-jpjc / CVE-2026-6322</strong> (host confusion via percent-encoded authority delimiters). <code>fast-uri</code> is pulled in transitively via <code>html-validate &rarr; ajv</code> and <code>stylelint &rarr; table &rarr; ajv</code>, and is used only by the local lint tooling &mdash; it is never bundled into the published site. <strong>No functional, UI, or user-facing changes</strong> to the Designer, Sizer, Switch Configuration, Reference Architectures page, or PowerPoint export. <code>npm audit</code> reports <strong>0 vulnerabilities</strong> after the bump; ESLint clean across all browser-facing scopes; all 1,156 existing unit tests still pass.</p>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    ...
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Polishes the <strong>Microsoft Sovereign Private Clouds reference architectures</strong> page (Knowledge tab) with five targeted refinements. <strong>AVD &harr; Disconnected guard</strong>: selecting <em>Azure Virtual Desktop</em> in step 1 now disables the <em>Disconnected / Air-gapped</em> card in step 2 (AVD requires the AVD control plane in Azure) and shows an inline warning; if Disconnected was already selected, connectivity is force-flipped back to <em>Connected</em>. <strong>Foundry Local cluster diagram</strong>: outer <strong>AKS Cluster</strong> container now nests an inner <strong>Foundry Local</strong> sub-container, with <strong>Edge RAG</strong> and <strong>Video Indexer</strong> tiles inside Foundry Local; the <em>AVAILABLE AI MODELS</em> strip moves above the workload band; GPUs are pulled out into a dedicated tile between the <em>SERVERS</em> title and the server pills, in the same green palette as the server pills (<code>#e6f5ee</code> / <code>#9bd6b8</code>) since GPUs are physical hardware. <strong>AVD Session Host Cluster</strong>: workloads simplified to <strong>AVD session hosts</strong>, <strong>FSLogix profiles</strong>, and <strong>AVD VMs</strong> (replacing <em>Domain controllers</em> + <em>AKS Arc</em>).</p>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.06</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 6, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Restyles the <strong>Configuration Report PowerPoint export</strong> (Designer's <em>Download PowerPoint</em> button) to match the dark-mode visual identity of the <em>Microsoft Sovereign Private Clouds reference architectures</em> PPTX export. The two ODIN-generated decks now share a single, consistent look-and-feel &mdash; dark <code>#0A0A0F</code> backgrounds, white titles with a blue accent underline, rounded-rect content cards with drop shadows, ODIN logo top-right on every section slide, and a centered cover slide with a large 2.2&Prime; logo. <strong>Bullet content and rasterized SVG diagrams are unchanged</strong> &mdash; only the layout, palette, and chrome were rebuilt. The deck is now assembled fully synchronously in the browser without a network round-trip to load the old <code>.potx</code> template.</p>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.05</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 6, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Adds an <strong>"About this architecture" slide</strong> to the PowerPoint export on the <em>Microsoft Sovereign Private Clouds reference architectures</em> page (Knowledge tab). The slide carries the same on-screen narrative &mdash; overview, control-plane explanation, and per-purpose design notes &mdash; so the deck now reads end-to-end <strong>Cover &rarr; About &rarr; Diagram &rarr; Control Plane &rarr; Summary &rarr; per-purpose detail slides</strong> instead of jumping straight from the cover into the diagram with no context. Body font auto-shrinks (12 &rarr; 11 &rarr; 10 &rarr; 9 pt) when the combined character count grows, so combined multi-purpose architectures still fit on a single 16:9 slide. Generated entirely client-side via the already-vendored JSZip 3.10.1.</p>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.04</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 6, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Adds <strong>Foundry Local on Azure Local (Preview)</strong>, <strong>Edge RAG Preview, enabled by Azure Arc</strong>, and <strong>Azure AI Video Indexer enabled by Arc (Preview)</strong> as three new top-level workload types in the Sizer, alongside Azure Local VMs, AKS Arc, and AVD. Foundry Local sizes per-replica model serving (Small SLM / Medium SLM / Large LLM / Custom + ONNX-GenAI or vLLM engine). Edge RAG sizes a turnkey 4-VM Retrieval Augmented Generation pipeline (LLM + embeddings + vector DB) in either GPU mode (4&nbsp;\u00d7 NC8_A2 / NC8_A16) or CPU mode, with vector-DB storage scaling from your document corpus size. AI Video Indexer sizes Microsoft's published Minimum / Recommended cluster-wide tiers for video and audio analysis on AKS Arc. All three workload types round-trip through JSON Export/Import, share URLs, the Designer hand-off, and Configuration Reports. Also adds a modern animated "Preview" banner in the top-left corner of each Preview workload-type button, and descriptive multi-line hover tooltips on every workload button.</p>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);"><strong>Other UX polish bundled in this release:</strong> Sizer AUTO sizing now enforces a <strong>24-core CPU minimum per node when GPUs are involved on multi-node clusters</strong> (Low Capacity stays capped at 14); fixed the misleading "<em>based on undefined requirements</em>" message when GPU was the bottleneck; Foundry model class examples now lead with <strong>OpenAI gpt-oss-20b</strong> (Medium) and <strong>OpenAI gpt-oss-120b</strong> (Large); Configuration Report renames <em>Video Indexer</em> &rarr; <strong>AI Video Indexer</strong>; mobile 3D rack visualization no longer overlaps the "Azure Local" brand badge; <strong>top navigation tab order changed to Knowledge | Sizer | Designer | ToR Switch</strong>; <em>Microsoft Sovereign Private Clouds Reference Architectures</em> is now the first item in the Knowledge tab; and the Reference Architectures page has a new <strong>"Share Architecture as URL"</strong> button (mirrors the Sizer's pattern, with property-key whitelisting on the URL-load path to satisfy CodeQL <code>js/prototype-polluting-assignment</code>).</p>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
