            ...
            <div style="color: var(--text-primary); line-height: 1.8;">
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.1); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    ...
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        ...
                        <li><strong>Release UI validation completed</strong> &mdash; full Sizer and Designer matrices cover supported topologies, transitions, imports, outputs, handoffs, resets, keyboard access, both themes, and responsive layouts. The pass fixed narrow-screen overflow, stale reset and architecture state, dialog Escape handling, and option-card disabled and nested-link semantics.</li>
                        <li>All <strong>1,575 / 1,575</strong> browser tests pass.</li>
                    </ul>
                </div>
                ...
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.1); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    ...
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        ...
                        <li><strong>Right-sized GPU inventory</strong> &mdash; automatically managed GPUs per machine are reconciled after machine scaling, including returning to zero after the final GPU workload is removed while preserving manual inventory.</li>
                        <li><strong>Stable scaling and reset behavior</strong> &mdash; Agentic Retrieval production defaults select a compatible GPU and synchronize Hardware Configuration; topology/rack transitions settle without recursive reversal; disaggregated rack capacity is rechecked; and ALDO Reset re-enables all workload cards.</li>
                        <li><strong>Minimum-fit procurement guidance</strong> &mdash; the first Sizing Note displays an amber &ldquo;Advisory&rdquo; label within the bold &ldquo;Advisory - minimum-fit hardware&rdquo; heading below 32 physical cores or 384 GB memory per machine.</li>
                        <li><strong>Single Node availability guidance</strong> &mdash; Single Node results place a matching Advisory first, explaining that one machine has no live-migration or workload-failover destination and restart-requiring maintenance interrupts workloads.</li>
                        ...
                        <li><strong>Infrastructure IP Pool auto-ending</strong> &mdash; entering a valid Starting IP in Designer automatically fills the minimum six-address Ending IP without overwriting a manual ending address, including after resume or import.</li>
                        <li><strong>Designer session and mobile reliability</strong> &mdash; valid Infrastructure IP Pool edits persist for resume, while compact navigation and aligned breadcrumbs keep utility controls accessible on narrow mobile screens.</li>
                        <li><strong>Designer UI and report reliability</strong> &mdash; completed SAN designs report architecture-aware selections without HCI-only false warnings; all report formats preserve workflow metadata, bounded Sizing Notes, and Advisory headings; the template picker is keyboard-operable; and the welcome screen identifies AI workloads in Sizer coverage.</li>
                        ...
                        <li>All <strong>1,560 / 1,560</strong> browser tests pass.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                ...
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.1); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    ...
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        ...
                        <li><strong>Storage Spaces Direct calculator</strong> &mdash; plan maximum supported volume size and storage-pool consumption for Azure Local and Windows Server, including resiliency, thin/fixed provisioning, single-tier and tiered disks, formulas, examples, text-report export, named shareable configuration URLs, the same physical-machine terminology used by Designer and Sizer, matching blue result values, a first-visit Help walkthrough, and local Resume or Start Fresh for changed configurations. Pool reporting separates physical available capacity from copy-adjusted customer-usable capacity and excludes cache drives from tiered capacity. The volume count rounds usable capacity divided by the individual-volume limit up, then uses the higher of that result or the machine count, keeping equal Fixed volumes within the supported maximum and creating at least one volume per machine. Azure Local automatically deducts the required 256 GB Infrastructure_1 and 20 GB ClusterPerformanceHistory volumes from usable capacity; Windows Server does not. Sizer applies the same 276 GB requirement throughout node and disk sizing, deducting it from hyperconverged S2D usable capacity or adding it to disaggregated SAN demand. Tiered layouts require at least as many capacity drives as cache drives. Fixed volumes receive an equal-size recommendation; Thin volumes explain on-demand pool consumption and display the current individual-volume limit. Defaults use 8 single-tier drives at 6.4 TB or 2 cache plus 4 capacity drives at 6.4 TB per machine.</li>
                        ...
                        <li>All <strong>1,501 / 1,501</strong> browser tests pass, including schema-drift, vendor-integrity, shared-branding, Azure Migrate, and S2D telemetry checks.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                ...
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    ...
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li><strong>iSCSI SAN (4-NIC) removed from the pickers</strong> &mdash; the DA1 storage-type card (Designer) and the disaggregated <em>Storage Connectivity</em> dropdown (Sizer) now list only <strong>Fibre Channel SAN</strong> and <strong>iSCSI SAN (6-NIC)</strong>.</li>
                        <li><strong>Legacy 4-NIC designs auto-upgrade to 6-NIC</strong> &mdash; a previously saved session, an imported Designer/Sizer JSON, or a Share-as-URL link that still carries <code>iscsi_4nic</code> is silently remapped to <code>iscsi_6nic</code> on load, so nothing reverts to the Fibre Channel default or lands on an unavailable option.</li>
                        <li><strong>Switch Config Quick Start iSCSI profile retired</strong> &mdash; the <em>Disaggregated &mdash; iSCSI</em> Quick Start builder (which scaffolded a 4-NIC cluster) has been removed; build iSCSI disaggregated clusters in the Designer instead. iSCSI QoS auditing and the disaggregated-iSCSI QoS profile are unchanged.</li>
                        <li><strong>6-NIC iSCSI + backup topology corrected</strong> &mdash; when In-Guest Backup is enabled on a 6-NIC iSCSI cluster, NIC3/NIC4 stay as <strong>standalone Cluster 1/2</strong> networks and the customer <strong>manually adds a Backup vNIC (VLAN 800) on top of the Management + Compute vSwitch</strong>. The previous design that built a <code>ClusterBackupSwitch</code> SET on NIC3/NIC4 has been removed from the host-networking diagrams (Designer and report), intent zones, port layouts, and QoS notes.</li>
                        <li><strong>iSCSI Fabric VLAN mode is now selectable</strong> &mdash; DA4 adds an <strong>Access/Trunk</strong> toggle for iSCSI Fabric A/B (paired, like the Cluster networks). The DA8 override VLAN fields are read-only and bound to DA4: <code>0</code> in Access mode, or the DA4 VLAN ID in Trunk mode.</li>
                        <li><strong>Backup Network is always Trunk</strong> &mdash; the In-Guest Backup vNIC always tags its VLAN, so the DA4 Backup Network row is fixed to Trunk (the Access option was removed).</li>
                        <li><strong>Fixed a false “missing subnet” error on Confirm Overrides</strong> &mdash; the DA8 Cluster/iSCSI subnet defaults are now seeded into state, so leaving them untouched no longer blocks confirmation (and the defaults flow through to the ARM export).</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.22.69</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">July 9, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Designer content improvement &mdash; the <strong>Private Endpoints</strong> step now lists the <strong>full set of Arc endpoints that cannot use Private Link</strong> (not just a single FQDN) and, when <em>Enabled</em> is selected, adds a proxy-bypass reminder linking to the Microsoft Learn proxy-configuration guide. Content/UI only; no logic or JSON-schema change.</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li><strong>All three unsupported Arc endpoints are now shown</strong> &mdash; the red card previously named only <code>his.arc.azure.com</code>. It now reads <strong>Arc Private Link Scopes</strong> and lists <code>*.his.arc.azure.com</code>, <code>*.guestconfiguration.azure.com</code>, and <code>*.dp.kubernetesconfiguration.azure.com</code>, with the clarified note that <em>Azure Arc Private Link Scopes are not supported by Azure Local</em>.</li>
                        <li><strong>Proxy-bypass reminder</strong> &mdash; when Private Endpoints are <em>Enabled</em>, an information box now appears below the service list: <em>&ldquo;When using a Proxy server, you <strong>must</strong> add your Private Endpoints to the <strong>Proxy Bypass List</strong> configuration of the physical machines &mdash; otherwise the private traffic will attempt to route through the proxy server,&rdquo;</em> linking to <a href="https://learn.microsoft.com/en-us/azure/azure-local/manage/configure-proxy-settings-23h2" target="_blank" rel="noopener">Configure proxy settings for Azure Local</a>.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.22.68</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">July 6, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Sizer bug fix &mdash; <strong>Share as URL</strong> now faithfully reproduces the shared configuration. A shared design (e.g. a disaggregated cluster with <strong>12 machines / 64 cores / 1024&nbsp;GB</strong>) previously reloaded as a different, larger design (<strong>22 machines / 32 cores / 512&nbsp;GB</strong>) because the page-load hardware initialisation reset the CPU configuration back to defaults <em>after</em> the shared config had been applied, then re-ran auto-scaling with the wrong core count. JSON <em>file</em> import was unaffected.</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li><strong>Share-as-URL round-trip fixed</strong> &mdash; on page load, <code>initHardwareDefaults()</code> ran unconditionally after a shared-URL config had already been applied, resetting the CPU generation and cores-per-socket (e.g. <strong>64&nbsp;&rarr;&nbsp;32</strong>) to their defaults; the follow-up recalculation then auto-scaled into a larger node count and lower per-node memory. The fresh-load initialisation is now skipped when a shared-URL config has been loaded, so the machine count, CPU, and memory match exactly what was shared.</li>
                        <li><strong>Disaggregated Storage Connectivity is now restored on import/resume</strong> &mdash; the <em>Storage Connectivity</em> choice (Fibre Channel vs iSCSI) was written to exports and shared URLs but never re-applied when loading, so an iSCSI SAN design silently reverted to the Fibre Channel default. It is now restored on both JSON import / Share-as-URL and on session resume.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.22.67</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">June 12, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Sizer bug fix (issue #261) &mdash; selecting <strong>Disaggregated Storage</strong> no longer sizes the <strong>internal disks</strong> to hold the workload&rsquo;s storage. On a disaggregated cluster the workload data lives on an <strong>external SAN</strong>, so the physical machines only need OS / boot disks. The Sizer now pins the internal <em>Physical Machine Storage Configuration</em> to a fixed <strong>2 &times; 0.96&nbsp;TB</strong> boot mirror and excludes internal storage from the node-count decision.</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li><strong>Internal disks are boot-only on Disaggregated</strong> &mdash; previously the auto-scaler grew the internal capacity disks to absorb the full workload storage (the same as Hyperconverged), so the greyed-out <em>Physical Machine Storage Configuration</em> &mdash; and the exported JSON &mdash; showed a proportional share of the SAN capacity as if it were local disk. It is now reset to a minimal <strong>2 &times; 0.96&nbsp;TB</strong> mirror that is never auto-scaled to the workload.</li>
                        <li><strong>Storage no longer drives node count on Disaggregated</strong> &mdash; the conservative node-scaling loop and storage-headroom checks now ignore internal disk utilisation for disaggregated clusters (SAN capacity is sized separately), so node count is decided purely by compute, memory, and GPU pressure.</li>
                        <li><strong>Switching Hyperconverged &rarr; Disaggregated cleans up</strong> &mdash; the internal disk fields are reset to the boot mirror as soon as the deployment type changes, so a design that was first sized as Hyperconverged no longer carries its inflated internal-disk values into the disaggregated view.</li>
                        <li><strong>4&nbsp;PB storage-limit warning now points to Disaggregated Storage &amp; gates the right action</strong> &mdash; when a Hyperconverged design&rsquo;s total raw cluster storage exceeds the 4&nbsp;PB (4,000&nbsp;TB) per-pool maximum, the storage-limit banner and sizing note now suggest switching the deployment type to <strong>Disaggregated Storage</strong> (external SAN, which removes the internal-storage limit) as well as the existing reduce-machines/disk-count/disk-size advice. The banner used to say &ldquo;export is blocked&rdquo; but only the Word export was gated &mdash; it now correctly reads that <strong>Configure in Designer</strong> is blocked (reports can still be exported), and the stray Word-export block was removed. The <strong>Configure in Designer</strong> button is also greyed out with a hover tooltip while the limit is exceeded, so the blocked action is obvious up-front rather than only on click.</li>
                        <li><strong>GitHub Enterprise Local is now Public Preview in the Knowledge tab</strong> &mdash; the GHEL reference-architecture card and its <em>About this architecture</em> write-up now read <strong>Public Preview</strong> (was <em>Private Preview</em>), matching the Sizer, and the narrative now includes a <strong>Learn more</strong> link to <a href="https://aka.ms/GHEL" target="_blank" rel="noopener">What is GitHub Enterprise Local? (preview)</a>. The <strong>Foundry Local</strong> card now also reads <strong>Public Preview</strong> (was a <em>New</em> badge) for consistency, with its own <strong>Learn more</strong> link to <a href="https://learn.microsoft.com/en-us/azure/azure-sovereign-clouds/private/foundry-local/what-is-foundry-local-on-azure-local" target="_blank" rel="noopener">What is Foundry Local on Azure Local?</a>, and the <strong>General Purpose Workloads</strong> tile description now opens with &ldquo;Azure Local VMs enable data residency, &hellip;&rdquo;.</li>
                        <li><strong>Knowledge tab pages are now shareable via the URL</strong> &mdash; opening the Knowledge tab and switching between its sub-pages (<em>reference architectures</em>, <em>Outbound Connectivity</em>, <em>AzLoFlows</em>) now reflects the current view in the address bar as <code>?tab=knowledge&amp;topic=&lt;slug&gt;</code>, so the link can be copied and shared. Previously the address bar always showed the bare root URL with no way to link to a specific Knowledge page. Opening a shared link now restores both the tab and the correct sub-page; the Designer tab still uses a clean root URL and the Sizer&rarr;Designer import flow is unaffected.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                ...
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    ...
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li><strong>New No/Yes dropdown: <em>Size hardware to accommodate 5 years of compound YoY growth</em></strong> (issue #254) inside the <code>&#x1F4C8; Capacity Runway</code> section. Defaults to <strong>No</strong>. Set to <strong>Yes</strong> and <code>getGrowthFactor()</code> switches from <code>(1 + pct/100)<sup>1</sup></code> to <code>(1 + pct/100)<sup>5</sup></code> &mdash; 10&nbsp;% becomes &times;1.61, 20&nbsp;% becomes &times;2.49, 30&nbsp;% becomes &times;3.71, 50&nbsp;% becomes &times;7.59. State persists through Resume, JSON import/export, Share-as-URL, and &#x1F504;&nbsp;Reset (returns to <strong>No</strong>).</li>
                        ...
                        <li><strong>Disaggregated &rarr; Hyperconverged auto-downgrade</strong> &mdash; symmetric inverse of the existing auto-upgrade. When the cluster was auto-upgraded to Disaggregated (the <em>Deployment Type</em> dropdown still carries its <strong>AUTO</strong> badge), and workloads are later removed so the design now fits in <strong>&le;&nbsp;12 machines on standard Hyperconverged</strong> (16 &times; 0.80 headroom), the Sizer now drops <em>Deployment Type</em> back to <strong>Hyperconverged</strong> in a single step (skipping the rack-by-rack shrink) and shows a toast. A 4-machine gap between the upgrade threshold (recommendation &gt;&nbsp;16) and the downgrade threshold (recommendation &le;&nbsp;12) gives a stable [13..16] band so workloads near the boundary do not ping-pong between the two deployment types. A user who manually changed the <em>Deployment Type</em> is never disturbed.</li>
                        <li><strong>AUTO badge on Deployment Type now persists across the post-upgrade recalc</strong> &mdash; the auto-upgrade fires once and re-runs the calculation; on that second pass the upgrade branch can't re-fire (we're already disaggregated) so the badge was being wiped by <code>clearAutoScaledHighlights()</code>. That also broke the new auto-downgrade, which needs the badge to know it's allowed to drop back to HCI &mdash; so e.g. reducing memory from 2&nbsp;TB to 1&nbsp;TB per node would auto-upgrade to Disaggregated as expected, but then removing the <strong>MANUAL</strong> override on memory would land at 2 racks of 7 nodes instead of going back to Hyperconverged. The badge now persists across the recalc (same re-apply pattern already used for <code>disagg-rack-count</code> and <code>future-growth</code>), so <strong>AUTO</strong> stays visible on <em>Deployment Type</em> and the symmetric downgrade fires correctly when the trigger condition disappears.</li>
                        <li><strong>AUTO-scaled field state now survives F5 / Resume, JSON Import, and Share-via-URL</strong> &mdash; the in-memory <code>_autoScaledFields</code> Set and <code>_disaggAutoUpgraded</code> guard were not persisted in the saved Sizer state or in exported JSON, so after pressing <strong>F5</strong> and clicking <strong>Resume</strong> &mdash; or after opening a Shared URL, or after importing an exported JSON &mdash; every AUTO badge was lost (including the cluster-type badge required by the auto-downgrade). Symptom: removing a <strong>MANUAL</strong> override on <em>Memory</em> after Resume / Import / URL-load left the cluster stuck on Disaggregated, instead of dropping back to Hyperconverged like it did in the same session before the reload. The saved state and exported JSON now carry <code>autoScaledFields</code> and <code>disaggAutoUpgraded</code>; both Resume and Import re-hydrate them before the next calculation, the AUTO badges reappear, and the symmetric auto-downgrade fires correctly. Schema updated; both fields are optional and backwards-compatible, so older JSON exports continue to import cleanly.</li>
                        <li><strong>UX polish</strong> &mdash; the optional electricity-price annual cost in the <em>Estimated Power, Heat &amp; Rack Space per Instance</em> panel now shows a small grey two-line <code>Annual cost:</code>&nbsp;/&nbsp;<code>(estimated)</code> label (matching the <code>Input:</code> label above it) so the value reads clearly as e.g. <em>Annual cost: 47,041/yr</em> instead of the bare <em>47,041/yr</em>. The label only appears when the price field is populated. The matching figure in the <strong>Multi-Instance Scale-Out</strong> panel is renamed to <em>Total Annual Cost:</em>&nbsp;/&nbsp;<em>(estimated)</em> (also two-line) for consistency and to stop the cell wrapping awkwardly. The <strong>Total Annual Energy</strong> figure (both single-instance and multi-instance) now auto-scales from <code>kWh/yr</code> to <code>MWh/yr</code> at &ge;&nbsp;1&nbsp;M&nbsp;kWh, so a 10&times; scale-out reads <em>1,605.7&nbsp;MWh/yr</em> instead of <em>1,605,710&nbsp;kWh/yr</em>. Also added a small vertical gap between the <code>&#x1F4C8; Capacity Runway</code> collapsible and the <em>Azure Local Instance &mdash; Sizing Notes</em> block so the two cards no longer butt up against each other. The Capacity Runway section header itself is also renamed to <code>&#x1F4C8; Capacity Runway &mdash; 5 Year-over-Year (YoY) Additional Workload Growth Projection</code> so the 5-year horizon and the <em>additional</em> nature of the growth (on top of today's workloads) are obvious from the header alone, with the <em>Allow for Future Growth</em> tooltip updated to match.</li>
                        <li><strong>Sizer polish &mdash; final pass for this release:</strong>
                            <ul style="margin: 4px 0 0 0; padding-left: 20px;">
                                <li><strong>Growth-applied banner</strong> under the four Total Requirements cards. When <em>Future Growth</em>&nbsp;&gt;&nbsp;0&nbsp;% or the new 5-year compound switch is <strong>Yes</strong>, a small yellow tip appears explaining where the multiplier came from &mdash; e.g. <em>&quot;Totals above include 20&nbsp;% YoY growth compounded over 5 years (&times;2.49) &mdash; hardware is sized for the full 5-year horizon.&quot;</em> &mdash; so the difference between raw workload demand and the hardware-sizing target is no longer invisible. Hidden when both growth&nbsp;=&nbsp;0&nbsp;% and years&nbsp;=&nbsp;1.</li>
                                <li><strong>Disaggregated auto-rack-scale toast wording</strong> now reads <em>&quot;exceeds N racks capacity&quot;</em> instead of <em>&quot;exceeds (N &times; 16)-machine capacity&quot;</em>, and reports the <strong>final</strong> rack count after the recursive recalc settles (a 6&nbsp;&rarr;&nbsp;7&nbsp;&rarr;&nbsp;8 cascade only shows the innermost toast).</li>
                                <li><strong>Auto rack-scale now climbs all the way to 8 racks</strong> (was hard-capped at 4). The cap was redundant because the caller already respects a MANUAL pin on <em>Number of Racks</em>. <strong>Crucially, the bump-walk skips rack counts that don't add capacity</strong> &mdash; per-rack cap is non-monotonic (1&ndash;4 racks &rarr; 16/rack&nbsp;=&nbsp;64 total; 5&nbsp;&rarr;&nbsp;60; 6&nbsp;&rarr;&nbsp;60; 7&nbsp;&rarr;&nbsp;63; 8&nbsp;&rarr;&nbsp;64), so from 5 racks (60) auto jumps straight to 7 (63) skipping the no-op 5&nbsp;&rarr;&nbsp;6 step. From 4 racks (already at the 64-machine architectural max) auto correctly does nothing &mdash; no later N has a higher cap.</li>
                                <li><strong>MANUAL-override capacity warning</strong> on Disaggregated now suggests <em>increasing Number of Racks</em> when it would genuinely help (over-utilised resource is Compute or Memory, <em>Number of Racks</em> is pinned at MANUAL, the cluster is at its per-rack cap, and a higher rack count exists whose total capacity is strictly greater than current). Stays silent when the only over-utilised resource is Storage (SAN-backed, more racks don't help) or when no later N would beat the current total cap.</li>
                                <li><strong>Multi-Instance Scale-Out &mdash; SAN-power note on Disaggregated</strong> &mdash; the multi-instance summary panel now shows the same <em>&quot;SAN storage appliance/array power is not included &mdash; consult your SAN vendor&quot;</em> callout that already appears on the per-instance power panel, but only when the deployment type is <em>Disaggregated Storage</em>, so the omission isn't lost when reading the cross-instance roll-up.</li>
                            </ul>
                        </li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    ...
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li><strong>Disaggregated rack auto-scale (up)</strong> &mdash; when the cluster type is <em>Disaggregated Storage</em>, the user has <strong>not</strong> manually pinned <em>Number of Racks</em>, and the conservative auto-scale loop has reached the per-rack machine cap (<code>rackCount &times; 16</code>) with utilisation still &ge;&nbsp;90&nbsp;% on any resource (CPU / memory / storage / GPU), the Sizer now bumps <em>Number of Racks</em> up (1 &rarr; 2 &rarr; 3 &rarr; 4, capped at 4 racks) <strong>before</strong> falling through to aggressive memory / ratio escalation. This is the &quot;prefer more racks over higher DIMMs&quot; weighting &mdash; adding a rack of cheap 2&nbsp;TB machines is preferred over jumping to 3-4&nbsp;TB DIMMs at the same machine count. The dropdown gets a persistent <strong>AUTO</strong> badge, <em>Number of Physical Machines</em> is rebuilt for the new rack count, and the Sizer re-runs the calculation. An info toast confirms the change. <strong>Going beyond 4 racks (5&ndash;8) stays a manual decision</strong> because that forces fewer machines-per-rack (5 &rarr; 12, 6 &rarr; 10, 7 &rarr; 9, 8 &rarr; 8) &mdash; a real fabric / floor-space change.</li>
                        <li><strong>Symmetric rack auto-shrink (down)</strong> &mdash; when workloads are removed or shrunk and the rack count was previously AUTO-set (user still hasn't pinned it), the Sizer now drops one rack tier per recalc if the workload fits at (<code>currentRacks &minus; 1</code>) racks with <strong>&ge;&nbsp;20&nbsp;% headroom</strong>. Single-step + 20&nbsp;% buffer prevents oscillation at the boundary (e.g. a workload right at 32 nodes won't ping-pong between 2 and 3 racks). Manually-chosen rack counts are never disturbed.</li>
                        <li><strong>Dropdown label rename: <em>Nodes per Rack</em> &rarr; <em>Physical Machines per Rack</em></strong> in the Sizer's Disaggregated Storage view, matching the wider v0.22.63 &quot;Nodes &rarr; Physical Machines&quot; terminology refresh for physical-server references. The dropdown <em>values</em> themselves (e.g. &quot;16 Nodes per Rack (32 total)&quot;) are intentionally <strong>unchanged</strong> because failover-cluster terminology still uses &quot;Node&quot;.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.22.63</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">June 4, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Splits <strong>Multi-Instance Scale-Out</strong> into its own metric pair &mdash; <strong>Total Annual Energy (kWh/yr)</strong> plus an optional, currency-agnostic <strong>Electricity Price &rarr; Total Annual Cost</strong> input independent of the per-instance one &mdash; and now <strong>persists</strong> the <em>Number of Instances</em> and both Electricity Price boxes across page reloads, <strong>Resume</strong>, &#x1F504; <strong>Reset</strong>, <strong>Share Sizer Config as URL</strong>, and JSON import/export. Also completes the <strong>Sizer terminology refresh</strong> from "Node" &rarr; "Physical Machine" for physical-server references, refreshes the <strong>first-visit Help tour</strong>, defaults <strong>Allow for Future Growth</strong> to <strong>10&nbsp;%</strong> with an AUTO/MANUAL badge, aligns <strong>Storage Resiliency</strong> wording (50&nbsp;% / 33&nbsp;% efficiency), and adds the three new Multi-Instance fields to the published Sizer JSON Schema.</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li><strong>Multi-Instance Scale-Out &mdash; Total Annual Energy &amp; optional Total Annual Cost</strong> &mdash; the multi-instance summary now shows a <strong>Total Annual Energy (kWh/yr)</strong> figure derived from per-instance annual energy &times; number of instances, plus a new optional, currency-agnostic <strong>Electricity Price (per kWh)</strong> input that drives a <strong>Total Annual Cost</strong> value (<code>per-instance annualKwh &times; instances &times; price</code>). The multi-instance price sits on its own line under the energy figure with an &quot;Input:&quot; hint and an &#9432; tooltip, and is fully independent of the per-instance Electricity Price already in the <em>Estimated Power, Heat &amp; Rack Space per Instance</em> panel &mdash; leave either, both, or neither populated.</li>
                        <li><strong>Persistence for Multi-Instance Scale-Out controls + the per-instance Electricity Price</strong> &mdash; the <em>Number of Instances</em> dropdown, the per-instance Electricity Price, and the multi-instance Electricity Price are now part of <code>getSizerState()</code> so they save to <code>localStorage</code> on every change and recalc, and are restored by both <strong>Resume</strong> (from the banner) and <strong>Apply</strong> (from JSON / URL import). That means <strong>Share Sizer Config as URL</strong> and <strong>Import / Export JSON</strong> also round-trip them. Previously they all reverted to defaults on reload.</li>
                        <li><strong>Sizer terminology refresh &mdash; "Nodes" &rarr; "Physical Machines"</strong> for physical-server references throughout the Sizer (user-facing labels, hardware-requirement summaries, sizing notes, power &amp; rack-unit breakdowns, GPU lock tooltips, host-overhead notes, Word export, and the 3D rack legend / per-server tile labels). <strong>Failover-cluster terminology stays as "Node"</strong> wherever it refers to the cluster role rather than the hardware (e.g. the <em>Single Node</em> deployment type, the <em>1&ndash;16 Nodes</em> / <em>N Nodes per Rack</em> dropdown values, <em>cluster node</em>, <em>witness node</em>, <em>storage node</em>, <em>worker node</em>, <em>AKS Arc node pools</em>).</li>
                        <li><strong>Sizer first-visit Help tour refreshed</strong> &mdash; the three-step onboarding modal now reflects current Sizer capabilities: the <em>Workload Modelling</em> tile lists every supported workload (VM, AKS Arc, AVD, GHEL, SQL, Foundry, File Server); the <em>Deployment Types</em> tile lists <strong>Single Node, Hyperconverged, Rack-Aware Cluster, Disaggregated Storage, or ALDO Management Cluster</strong> (matching the actual dropdown order); <em>Auto-Save &amp; Import/Export</em> mentions the <strong>RVTools</strong> estate import; the <em>Pro Tips</em> page documents the <strong>10&nbsp;% Future Growth default</strong>, the <strong>Multi-Instance Scale-Out</strong> view, and the <em>Auto-Scaling</em> rule that the engine grows cores / memory / disks per machine before adding more machines. Onboarding key bumped so existing visitors see the refreshed tour once.</li>
                        <li><strong>Sizer &quot;Allow for Future Growth&quot; now defaults to 10&nbsp;% (recommended)</strong> &mdash; first-time visitors see a realistic sizing envelope instead of <em>No additional growth</em>. The dropdown shows an <strong>AUTO</strong> badge on fresh load so it's clear the 10&nbsp;% is a recommended default; any change &mdash; including switching back to <em>No additional growth</em> &mdash; replaces it with a <strong>MANUAL</strong> badge so an explicit &quot;no growth&quot; choice is preserved and visible. Dismissing the MANUAL badge with &times; reverts the dropdown back to the <strong>AUTO 10&nbsp;%</strong> default. Restoring a previous Sizer session leaves the saved value untouched and shows no badge.</li>
                        <li><strong>Storage Resiliency dropdown wording aligned</strong> &mdash; <em>Two-way Mirror (50% efficiency)</em> / <em>Three-way Mirror (33% efficiency)</em> now match the existing <em>Four-way Mirror (25% efficiency)</em> (rack-aware) and <em>Three-way Mirror (33% efficiency)</em> (ALDO Management) labels in both the static 2-node default and the dynamic 2-node rebuild path. The previous mixed <em>(min N nodes)</em> / <em>(N% efficiency)</em> wording was inconsistent.</li>
                        <li><strong>&quot;Info: Sizer hardware scaling weighting logic&quot; link moved to its own line</strong> under the <em>Physical Machine(s) &mdash; Example Hardware Configuration</em> section description so it's easier to spot and no longer crowds the first sentence.</li>
                        <li><strong>&#x1F504; Reset now clears Multi-Instance Scale-Out controls</strong> back to their defaults (Number of Instances &rarr; 1, per-instance Electricity Price &rarr; blank, multi-instance Electricity Price input &rarr; blank, cached multi-instance price &rarr; empty), matching the rest of <code>resetScenario()</code>'s clean-slate behaviour.</li>
                        <li><strong>Three new top-level fields documented in the published Sizer JSON Schema</strong> (<code>docs/json-schema/odin-sizer.schema.json</code>) &mdash; <code>instanceCount</code>, <code>powerPriceInput</code>, <code>multiPowerPriceInput</code>. Backwards-compatible (the fields are optional). The schema-drift CI guard picks them up automatically as part of <code>getSizerState()</code>.</li>
                        <li><strong>Fix: Sizer Number of Physical Machines dismiss-&times; now cascades cleanly to resiliency</strong> &mdash; clicking the dismiss-&times; on the <strong>MANUAL</strong> badge for <em>Number of Physical Machines</em> now resets the dropdown to the current cluster type's minimum (1 / 2 / 3) <strong>and</strong> re-runs the resiliency-options + cluster-info refresh, so the <strong>Storage Resiliency</strong> dropdown immediately reflects the new node count (e.g. a 3-way mirror that's no longer valid at 2 machines is removed) instead of leaving a stale combination on screen.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.22.62</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">June 3, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Adds <strong>GitHub Enterprise Local (GHEL)</strong> as a first-class Sizer workload type with tier-driven sizing and Advanced HA replica configuration, introduces a <strong>Sizer hardware scaling weighting logic</strong> info popup so users can see how the auto-scaler picks node count / memory / ratio / cluster type, and adds a <strong>memory-density 2-node &rarr; 3-node soft preference</strong> so a high-RAM 2-node cluster is auto-bumped to 3 nodes for a stronger 3-way mirror.</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li><strong>GitHub Enterprise Local (GHEL) workload</strong> &mdash; new Sizer tile with <strong>tier-driven sizing</strong> (Trial through 8000&ndash;10000 seats from the GHES minimum recommended requirements), HA Yes/No basic dropdown, and an <em>Advanced configuration</em> panel for picking 0&ndash;7 replicas (1 primary + N) with a link to the GHES HA docs and a throughput note. Workload-card detail line shows the topology (HA pair, Primary + N replicas) and includes a <code>(sizing info)</code> link straight to the GHES reference.</li>
                        <li><strong>&quot;Info: Sizer hardware scaling weighting logic&quot; popup</strong> &mdash; new inline blue link in the <em>Physical Node(s) &ndash; Example Hardware Configuration</em> section opens a modal that documents the auto-scaler's rules in order: prefer-more-nodes-over-bigger-nodes (1.5 TB / 2 TB memory cap), cores &rarr; sockets &rarr; ratio escalation, two-pass memory limits (1.5 TB conservative / 4 TB aggressive), the new 768 GB / 2-node &rarr; 3-node preference, the auto-upgrade to Disaggregated Storage at 16 nodes, and the step-back-down reduction loop &mdash; plus an explicit override note: every auto-decision is advisory and manually setting any field locks that choice.</li>
                        <li><strong>2-node memory-density 3-node preference</strong> &mdash; when the conservative auto-scale pass lands on exactly 2 nodes but per-node memory has climbed above 768 GB, the Sizer now auto-bumps to 3 nodes and re-runs auto-scale so per-node memory drops to the next-lower DIMM tier. This unlocks a <em>three-way mirror</em> (full data redundancy after any single node failure) and spreads workloads across smaller, cheaper DIMMs. Only runs when conservative scaling already succeeded, the user has not pinned the node count, and the 3-node solution stays under 90&nbsp;% utilisation; otherwise the Sizer reverts to 2 nodes.</li>
                        <li><strong>SIZER_VERSION bumped 1 &rarr; 2</strong> to reflect the new <code>tier</code> / <code>ha</code> / <code>replicas</code> fields on GHEL workloads in the Sizer export payload. The importer continues to accept v1 payloads (forward-compatible). ESLint clean; HTML validation clean; full suite passes <strong>1,325 / 1,325</strong>.</li>
                        <li><strong>Foundry Local references refreshed for Build 2026</strong> &mdash; updated the SLM / LLM model-class examples in the Sizer to the current Foundry Local catalog (Phi-4, Phi-4-reasoning, gpt-oss-20b, gpt-oss-120b (vLLM-only), Mistral-7B-v0.2, DeepSeek-R1 7b/14b, Qwen3, NVIDIA Nemotron, Whisper), clarified the <strong>ONNX-GenAI (CPU or GPU) vs vLLM (GPU only)</strong> engine choice, called out <strong>multi-node inference</strong> scheduling across cluster-wide GPU / CPU capacity, and refreshed the <em>Learn more</em> links block &mdash; now includes <a href="https://learn.microsoft.com/en-us/azure/azure-sovereign-clouds/private/foundry-local/what-is-foundry-local-on-azure-local" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">What is Foundry Local on Azure Local?</a>, <a href="https://learn.microsoft.com/en-us/azure/azure-sovereign-clouds/private/azure-local/ai-workloads-overview" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">AI workloads on Azure Local (overview)</a>, the <a href="https://aka.ms/build26blog" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">Build 2026 announcement</a>, <a href="https://aka.ms/FoundryLoca_Techcommunity_Build_blog" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">Multi-node inference, vLLM &amp; expanded model catalog</a>, and <a href="https://aka.ms/AgentsAndToolsBuildBlog2026" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">Agentic Retrieval, Knowledge &amp; Chat UI in Foundry Local</a>.</li>
                        <li><strong>Privacy footer refreshed + analytics opt-out added</strong> &mdash; the footer on all 5 pages (Designer, Sizer, ARM, Report, ToR Switch) now clearly states that user inputs / configurations / template data stay in the browser and discloses that anonymous integer-only usage counters (page views and feature use, no IP / identifier) are incremented to populate the header stats. A new <strong>Page views opt-out</strong> checkbox sits in the top navigation bar (right of the light/dark theme toggle) &mdash; preference persisted in <code>localStorage</code> (<code>odin-analytics-opt-out=1</code>) &mdash; and analytics also auto-disables when the browser sends <code>navigator.doNotTrack</code> or <code>navigator.globalPrivacyControl</code>. Footer links to the <a href="https://github.com/Azure/odinforazurelocal/blob/main/js/analytics.js" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">analytics source</a> for transparency.</li>
                        <li><strong>Fix: Sizer &quot;VM cannot be placed on a single machine&quot; false positive for Total VM Requirements input</strong> &mdash; the per-VM placement validator compared the workload's <code>vcpus</code> / <code>memory</code> fields against per-node capacity, but in <strong>Total VM Requirements</strong> input mode those fields hold the fleet-wide totals (with <code>count = 1</code>), not the spec of any individual VM. A modest 1000-VM fleet totalling 4000 vCPUs / 16&nbsp;000 GB was therefore flagged with a red &quot;&#x1F6AB; This VM cannot be placed on a single machine&quot; note, fired the <em>&quot;One or more VMs exceed per-machine capacity&quot;</em> toast, and blocked the <strong>Configure in Designer</strong> button. The check now runs only for <code>inputMode === 'per-vm'</code> workloads, where per-VM semantics actually hold.</li>
                        <li><strong>Sizer 3D Hardware Visualization &mdash; consistent default zoom for single-rack deployments</strong>, regardless of server / node count. Previously the single-rack camera distance scaled by node count, so the empty default 2-node page looked noticeably smaller / further away than the post-calculation view of a busy 8&ndash;16-node rack. Single-rack mode now uses one fixed camera distance so the full 42U rack body (plus the core switch above) fills the frame identically on first load and after any calculation. Pure camera-distance tweak; look-at target, tilt, camera angle, and the multi-rack / rack-aware / disaggregated / two-row layouts are all unchanged.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.22.61</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">June 2, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Documents ODIN's export/import format as machine-readable <strong>JSON Schemas</strong> and hardens the importers (<a href="https://github.com/Azure/odinforazurelocal/issues/237" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #237</a>). The Designer and Sizer each get their own draft-07 schema so designs can be validated and automated offline. Also bundles a few Sizer import/reset UX fixes.</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li><strong>Two JSON Schemas</strong> &mdash; one for the Designer export (<code>version</code> / <code>exportedAt</code> / <code>state</code>) and one for the Sizer export (<code>_meta</code> / <code>data</code>, with a <code>workloads</code> model keyed by workload type). Both forward-compatible, self-documenting, and published so they can be referenced by URL in a <code>$schema</code> key.</li>
                        <li><strong>Format docs</strong> &mdash; how to validate an ODIN export from CI, Terraform, or any language (ajv, python-jsonschema, VS Code) with no ODIN code required.</li>
                        <li><strong>Schema-drift tests</strong> &mdash; CI fails if a Designer or Sizer state field changes without the matching schema being updated, so the published schemas stay accurate.</li>
                        <li><strong>Import hardening</strong> &mdash; prototype-pollution guard (Designer) and workload-shape validation (Sizer), with non-blocking warnings for unknown fields so existing files keep working.</li>
                        <li><strong>Sizer Estimated Power &mdash; Annual Energy (kWh/yr) + optional running cost</strong> &mdash; the <em>Estimated Power, Heat &amp; Rack Space</em> panel now shows an estimated yearly energy figure (total Watts run 24/7/365) and an optional, currency-agnostic <strong>Electricity Price (per kWh)</strong> box that estimates the annual running cost in whatever currency you enter (with an &#9432; hover tip), alongside the existing per-machine Watts, total Watts, and BTU/hr.</li>
                        <li><strong>Sizer import / reset UX fixes</strong> &mdash; importing a Sizer Configuration JSON now confirms with a toast; the RVTools cluster picker no longer forces a selection (you can clear the last cluster); and <em>Reset</em> from a Disaggregated Storage config now properly hides the <em>Number of Racks</em> / <em>Storage Connectivity</em> rows.</li>
                        <li><strong>3D Sizer SAN overlap &amp; disaggregated text fixes</strong> (<a href="https://github.com/Azure/odinforazurelocal/issues/245" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #245</a>) &mdash; the 3D rack view no longer draws the first node overlapping the SAN appliance by 1U on Disaggregated Storage deployments, and the <em>host compute overhead</em> assumptions text no longer references the S2D stack / Software Storage Bus / storage-repair headroom for external-SAN hosts (the reservation math is unchanged).</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.22.55</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">June 2, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Adds <strong>RVTools Excel import</strong> to the Sizer (<a href="https://github.com/Azure/odinforazurelocal/issues/230" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #230</a>) &mdash; turn a VMware RVTools <code>.xlsx</code> export into Sizer workloads in a few clicks, entirely client-side &mdash; and fixes a node-count sizing bug.</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li><strong>RVTools import tab + &#x1F4CA; toolbar button</strong> &mdash; reads <code>vInfo</code> (and <code>vCluster</code> / <code>vHost</code> when present), summarises each source cluster (VMs, vCPU, memory, storage, hosts) with a whole-file totals banner.</li>
                        <li><strong>Pick one or more clusters</strong> &mdash; selecting several consolidates into one cluster (grouped bands merge; per-VM lists concatenate), seeding a generic <em>Consolidated</em> name. A <strong>select-all</strong> checkbox in the picker header ticks every cluster at once.</li>
                        <li><strong>Two modes</strong> &mdash; Grouped (bands by vCPU / RAM size class) and Per-VM (one workload per VM). Privacy: VM names appear <strong>only</strong> in Per-VM mode and no RVTools data leaves the browser.</li>
                        <li>Choose <strong>Provisioned</strong> or <strong>In Use</strong> storage, optionally include powered-off VMs (templates excluded). A new editable <strong>Cluster Name</strong> field carries through <strong>Sizer &rarr; Designer &rarr; ARM</strong>. Excel parsing via <strong>SheetJS Community 0.20.3</strong>, vendored locally and lazy-loaded with a pinned SHA-256.</li>
                        <li><strong>Fixed</strong>: the auto-scale node loops now use the same S2D-aware host reserve as the <em>&gt;90% capacity</em> banner, so a recommended config no longer trips its own warning on high-memory nodes (e.g. 1536 GB/node now recommends 3 nodes, not 2 at 95%).</li>
                        <li>Clearer Sizer labels (<em>Total</em> &rarr; <em>Required</em>, a <em>Number of Workloads</em> box, the per-node heading now states the active <em>X:1</em> overcommit ratio). ESLint clean; HTML validation clean; full suite passes <strong>1,314 / 1,314</strong>.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.22.01</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">June 1, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Improves the Sizer's <strong>physical-host compute reserve / overhead model</strong> (<a href="https://github.com/Azure/odinforazurelocal/issues/232" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #232</a>), reorients the <strong>rack-layout diagrams</strong> bottom-up (<a href="https://github.com/Azure/odinforazurelocal/issues/233" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">#233</a>), removes <strong>Low Capacity</strong> (<a href="https://github.com/Azure/odinforazurelocal/issues/234" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">#234</a>), and refines the <strong>imported Azure Local instance</strong> experience (<a href="https://github.com/Azure/odinforazurelocal/issues/235" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">#235</a>).</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li><strong>S2D-aware host reserve</strong> &mdash; sized by cluster type and storage shape instead of a flat percentage: CPU <code>max(ceil(20%), 2)</code> (ALDO management) / <code>max(ceil(10%), 1)</code> (disaggregated) / <code>max(ceil(10%), 2)</code> (standard S2D); memory base 32 GB (S2D) or 24 GB (disaggregated) + <code>ceil(4 GB &times; cacheTB)</code> + 64 GB when ALDO management is hosted, floored at 8% of host RAM. A new collapsible <em>assumptions &amp; math</em> section shows the tables.</li>
                        <li><strong>Bottom-up rack diagrams</strong> &mdash; all four <code>report/rack-svg.js</code> generators, the 3D view, and the wizard preview stack servers from the bottom up (switches stay at the top); real Designer node names surface via new <code>getRackNodeLabel()</code> / <code>getRackTorLabel()</code> helpers.</li>
                        <li><strong>Low Capacity removed</strong> (#234) &mdash; gone from Designer and Sizer (Designer now offers 5 templates) and the cluster-type mapping helpers.</li>
                        <li><strong>Imported instance refinements</strong> (#235) &mdash; imported CPU / memory / disk / node count locked to MANUAL; the overcommit ratio still escalates (4&rarr;5&rarr;6) on a fixed cluster; a new <strong>Disaggregated Storage</strong> import option greys out the S2D disk picker; the import disk-size dropdown matches the Sizer; <em>Parse &amp; Preview</em> hides after a clean parse.</li>
                        <li><strong>Report security controls</strong> &mdash; the report always lists all seven controls as Enabled / Disabled. ESLint clean; HTML validation clean; all <strong>1,240 / 1,240</strong> tests pass.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.14</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 20, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Adds the <strong>Microsoft 365 Local &mdash; Medium-Scale</strong> reference architecture to the <em>Microsoft Sovereign Private Clouds</em> page (Knowledge tab), and groups co-located single-node clusters into shared rack cards.</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li><strong>Medium-Scale</strong>: 2 single-node Azure Local clusters hosting Exchange mailbox servers (one shared rack card) + 1 three-node cluster hosting Exchange Edge Transport, SharePoint, Skype for Business and SQL &mdash; 3 clusters / 5 servers, drawn as 2 cards.</li>
                        <li><strong>Large-Scale refinement</strong>: the 4 single-node Exchange mailbox clusters now share one card (Servers 1-4) and the 2 Edge Transport clusters share another (Servers 5-6); the 3-node cluster keeps its own card.</li>
                        <li>Inside each merged card, every server sits in its own dashed <em>Cluster N</em> sub-frame, making clear each is an independent single-node cluster (own quorum + S2D pool).</li>
                        <li>AD, Firewall, Load Balancer and the management-network router remain infrastructure components, not separate clusters. Storage fixed at <strong>S2D (local)</strong>.</li>
                        <li>The on-screen SVG and PowerPoint export pick up the variant automatically via <code>tpl.scaleVariants</code>. ESLint clean; HTML validation clean; full suite passes.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.13</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 12, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Cosmetic fix on the <strong>Microsoft Sovereign Private Clouds</strong> reference architectures page (Knowledge tab).</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li>The <em>Azure Local Cluster (up to 128 nodes)</em> title no longer shows a misleading <code>*</code> (Coming soon) suffix &mdash; 128 nodes is documented and supported.</li>
                        <li>Removed the stray <code>comingSoon: true</code> flag from the <code>cluster-128</code> entry in <code>cardForScale()</code>; both the SVG diagram and the PowerPoint export render the title cleanly.</li>
                        <li>The unrelated <em>logical-isolation</em> tenancy <em>Coming soon</em> badge was left untouched. ESLint clean; HTML validation clean; all <strong>1,246 / 1,246</strong> tests pass.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.12</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 12, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Two bug fixes plus a wizard quality-of-life feature &mdash; closes <a href="https://github.com/Azure/odinforazurelocal/issues/221" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #221</a> and <a href="https://github.com/Azure/odinforazurelocal/issues/223" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #223</a>.</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li><strong>Designer &middot; Step 15 "&#x1FA84; Auto-fill from Node IPs"</strong> &mdash; one click derives the Infrastructure CIDR, Default Gateway, and Infra IP Pool start/end from the Step 12 Node IPs; gateway prefers <code>.1</code>, the pool is 6 IPs placed past the highest node IP with headroom; manually edited fields are kept and reported in a toast.</li>
                        <li><strong>Designer &middot; Step 15 live "Infrastructure Subnet Utilisation" diagram</strong> &mdash; a horizontal bar (Default Gateway / Infra IP Pool / Node IPs / Unused) now renders inline as you type; the same SVG is added to the Configuration Report. Single source of truth in new <code>js/subnet-utilization.js</code>.</li>
                        <li><strong>Disaggregated rack diagram &mdash; single-rack overlap/clipping fixed</strong> in both the wizard preview (<code>js/disaggregated.js</code>) and the report (<code>report/rack-svg.js</code>): both now compute a content-aware width floor, centre the rack column, and span the SAN/iSCSI boxes full width.</li>
                        <li>ESLint clean; HTML validation clean; all <strong>1,246 / 1,246</strong> tests pass.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.11</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 12, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);"><strong>Sizer hardware-options alignment with the Azure Local Solutions Catalog</strong> (closes <a href="https://github.com/Azure/odinforazurelocal/issues/226" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #226</a>).</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li>New build-time gap check (<code>scripts/catalog-gap-check.js</code>) compares every Sizer dropdown against a captured catalog snapshot and writes a gap report after the test suite.</li>
                        <li>Adds <strong>NVIDIA A100</strong> (80 GB / 300 W / MIG up to 7 slices &mdash; stops at <code>1/8</code>, no <code>1/16</code>) and <strong>NVIDIA A40</strong> (48 GB / 300 W / vGPU up to 16 partitions).</li>
                        <li><strong>xeon-5th</strong> extended to <strong>128 cores/socket</strong>; <strong>xeon-6</strong> adds <strong>36</strong> and <strong>80</strong> core options.</li>
                        <li><strong>40-drive capacity nodes</strong> (single- and two-tier); capacity sizes <strong>8 / 12 / 16 / 20 TB</strong>; cache size <strong>12.8 TB</strong>. Memory ceiling intentionally held at 4 TB/node.</li>
                        <li>ESLint clean; HTML validation clean; all <strong>1,171 / 1,171</strong> tests pass.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.10</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 12, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Polish release for the <strong>Microsoft Sovereign Private Clouds reference architectures</strong> page (Knowledge tab) and its PowerPoint export.</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li><strong>Tenant header alignment unified</strong> &mdash; strict and logical-isolation cards share the same label-above + dashed-border + chip layout so workload areas align across a row.</li>
                        <li><strong>SAN-using clusters grouped together</strong> &mdash; a stable sort moves SAN clusters to the left of each band so the SAN cylinder anchors only those cards.</li>
                        <li><strong>Microsoft 365 Local now uses the official M365 SVG logo</strong> (public-domain, Wikimedia Commons) on both the card and the PPT hero.</li>
                        <li><strong>PPT overview slide</strong> uses shorter scale labels in a wider auto-shrinking pill; <strong>PPT per-purpose slide</strong> gains compact TENANCY / STORAGE tiles + a wide SCALE panel. "RECOMMENDED" badge removed from the connectivity step.</li>
                        <li>ESLint clean; HTML validation clean; all 1,160 tests pass; PPTX smoke test valid.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.09</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 12, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);"><strong>Bug fix</strong> &mdash; resolves <a href="https://github.com/Azure/odinforazurelocal/issues/222" target="_blank" rel="noopener noreferrer" style="color: var(--accent-blue);">issue #222</a> on Designer Step 16 (<em>Storage Pool Configuration</em>).</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li>The <strong>SAN Volume LUN ID</strong> inputs were shown whenever <strong>InfraOnly</strong> was selected, regardless of architecture &mdash; but they only apply to the <strong>Disaggregated</strong> (<code>create-cluster-san</code>) template, so Hyperconverged users couldn't fill them in.</li>
                        <li>The visibility check in <code>syncStoragePoolUi()</code> is now gated on <code>state.architecture === 'disaggregated'</code> only; HCI users (Express / InfraOnly / KeepStorage) see only the storage-pool choice cards.</li>
                        <li>Purely a DOM-visibility fix &mdash; no state-machine, validation, or ARM-generation changes. New test covers all four (architecture &times; config) combinations. ESLint clean; all 1,160 tests pass.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.08</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 11, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);"><strong>Security maintenance release</strong> &mdash; no functional or user-facing changes.</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li>Bumps the transitive <code>fast-uri</code> dependency to <code>&gt;= 3.1.2</code> via a <code>package.json</code> override, clearing two high-severity Dependabot alerts: <strong>CVE-2026-6321</strong> (path traversal) and <strong>CVE-2026-6322</strong> (host confusion).</li>
                        <li><code>fast-uri</code> is pulled in transitively (<code>html-validate &rarr; ajv</code>, <code>stylelint &rarr; table &rarr; ajv</code>) and used only by local lint tooling &mdash; never bundled into the published site.</li>
                        <li><code>npm audit</code> reports <strong>0 vulnerabilities</strong>; ESLint clean; all 1,156 tests still pass.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.07</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 6, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Polishes the <strong>Microsoft Sovereign Private Clouds reference architectures</strong> page (Knowledge tab) with targeted refinements.</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li><strong>AVD &harr; Disconnected guard</strong>: choosing <em>Azure Virtual Desktop</em> disables the <em>Disconnected / Air-gapped</em> card (AVD needs the Azure control plane) and force-flips connectivity back to <em>Connected</em> if needed.</li>
                        <li><strong>Foundry Local cluster diagram</strong>: the AKS Cluster container nests an inner Foundry Local sub-container with Edge RAG / Video Indexer tiles; GPUs get a dedicated tile in the physical-hardware palette.</li>
                        <li><strong>AVD Session Host Cluster</strong>: workloads simplified to AVD session hosts, FSLogix profiles, and AVD VMs.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.06</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 6, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Restyles the <strong>Configuration Report PowerPoint export</strong> (Designer's <em>Download PowerPoint</em> button) to match the dark-mode identity of the reference-architectures PPTX export.</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li>Both ODIN decks now share one look &mdash; dark <code>#0A0A0F</code> backgrounds, white titles with a blue accent underline, rounded content cards with shadows, ODIN logo top-right, and a centered cover with a large 2.2&Prime; logo.</li>
                        <li><strong>Bullet content and rasterized SVG diagrams are unchanged</strong> &mdash; only layout, palette, and chrome were rebuilt.</li>
                        <li>The deck is now assembled fully in the browser with no network round-trip to the old <code>.potx</code> template.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.05</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 6, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Adds an <strong>"About this architecture" slide</strong> to the PowerPoint export on the <em>Microsoft Sovereign Private Clouds reference architectures</em> page (Knowledge tab).</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li>Carries the same on-screen narrative (overview, control-plane explanation, per-purpose notes), so the deck reads <strong>Cover &rarr; About &rarr; Diagram &rarr; Control Plane &rarr; Summary &rarr; per-purpose</strong>.</li>
                        <li>Body font auto-shrinks (12 &rarr; 11 &rarr; 10 &rarr; 9 pt) as content grows, so multi-purpose architectures still fit one 16:9 slide.</li>
                        <li>Generated entirely client-side via the already-vendored JSZip 3.10.1.</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--accent-blue);">Version 0.21.04</h4>
                    <div style="font-size: 13px; color: var(--text-secondary);">May 6, 2026</div>
                    <p style="margin: 8px 0 0 0; font-size: 13px; color: var(--text-secondary);">Adds three new top-level <strong>AI workload types</strong> to the Sizer &mdash; <strong>Foundry Local (Preview)</strong>, <strong>Edge RAG (Preview, enabled by Azure Arc)</strong>, and <strong>Azure AI Video Indexer enabled by Arc (Preview)</strong> &mdash; alongside Azure Local VMs, AKS Arc, and AVD.</p>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; font-size: 13px; color: var(--text-secondary);">
                        <li><strong>Foundry Local</strong> sizes per-replica model serving (Small SLM / Medium SLM / Large LLM / Custom + ONNX-GenAI or vLLM engine); examples lead with OpenAI gpt-oss-20b (Medium) and gpt-oss-120b (Large).</li>
                        <li><strong>Edge RAG</strong> sizes a turnkey 4-VM RAG pipeline (LLM + embeddings + vector DB) in GPU mode (4&nbsp;&times; NC8_A2 / NC8_A16) or CPU mode, with vector-DB storage scaling from your corpus size.</li>
                        <li><strong>AI Video Indexer</strong> sizes Microsoft's published Minimum / Recommended cluster-wide tiers on AKS Arc. All three round-trip through JSON, share URLs, the Designer hand-off, and Reports, with animated <em>Preview</em> banners and hover tooltips.</li>
                        <li><strong>UX polish</strong>: AUTO sizing enforces a 24-core/node minimum when GPUs are on multi-node clusters; fixed the misleading "based on undefined requirements" GPU message; mobile 3D rack no longer overlaps the brand badge.</li>
                        <li>Top navigation reordered to <strong>Knowledge | Sizer | Designer | ToR Switch</strong>; the reference-architectures page is now first in Knowledge and gains a <strong>"Share Architecture as URL"</strong> button (with property-key whitelisting to satisfy CodeQL <code>js/prototype-polluting-assignment</code>).</li>
                    </ul>
                </div>
                <div style="margin-bottom: 24px; padding: 16px; background: rgba(59, 130, 246, 0.05); border-left: 4px solid var(--accent-blue); border-radius: 4px;">
                ...
                <div style="margin-bottom: 24px; padding-bottom: 24px; border-bottom: 1px solid var(--glass-border);">
                    <h4 style="color: var(--accent-purple); margin: 0 0 12px 0;">🛡️ Security — CodeQL alerts cleared</h4>
                    <ul style="margin: 0; padding-left: 20px;">
                        <li><strong><code>js/xss-through-dom</code> (alert #15, <code>sizer/sizer.js</code>)</strong>: Sizer <em>Parse &amp; Preview</em> for an Azure Local machine JSON paste now coerces every JSON-derived numeric value (<code>coreCount</code>, <code>coresPerSocket</code>, <code>sockets</code>, <code>memoryGiB</code>) to <code>Number()</code> immediately before they are interpolated into the preview HTML — making the type-narrowing barrier explicit on the data-flow path from <code>&lt;textarea&gt;.value</code> → <code>JSON.parse(...)</code> → <code>previewDiv.innerHTML</code>. String values were already escaped via <code>escapeHtml()</code>.</li>
