...
- **ip-address**: NEVER allow `ip-address` to regress below version **10.3.1**. Earlier 10.x releases contain XSS and address-classification flaws that can bypass SSRF or trust-boundary checks. Pulled in transitively as a dev-only dependency through Puppeteer; maintain the `">=10.3.1"` override.
- **fast-uri**: NEVER allow `fast-uri` to regress below version **4.1.4**. Versions below 4.1.3 contain high-severity host-confusion and SSRF flaws (GHSA-5jgf-p345-68v8, GHSA-f65p-4m7j-42xc, GHSA-fph4-wmhf-6fwf, and GHSA-jqff-g426-hqxp); 4.1.4 also rejects malformed IPv6 zone identifiers. Pulled in transitively through HTML Validate and Stylelint; maintain the `">=4.1.4"` override.
- **brace-expansion**: NEVER allow the **5.x** line below **5.0.9** or the **1.x** line below **1.1.18**. Maintain both selector overrides; do not replace them with name-nested overrides.
- **js-yaml**: NEVER allow `js-yaml` to regress below version **4.3.2**. Earlier 4.x releases have denial-of-service advisories, including the empty-merge-source CPU limit bypass in 4.3.1 (GHSA-2883-xcg3-v3hh). Pulled in transitively through ESLint and Puppeteer tooling; maintain the `">=4.3.2 <5.0.0"` override.
