...
- **fast-uri**: NEVER allow `fast-uri` to regress below version **4.1.1**. Version 4.1.1 remains affected by GHSA-7p8r-x3mc-p8w7; patched version 4.1.2 is not yet available through the configured feed. The exact advisory is temporarily allowed for this dev-only package. Remove the exception and raise the override when 4.1.2 becomes available.
- **brace-expansion**: NEVER allow the **5.x** line below **5.0.7** or the **1.x** line below **1.1.16**. GHSA-mh99-v99m-4gvg and GHSA-rgw5-rvv9-x895 are temporarily allowed because patched 5.0.9 and 1.1.18 releases are not yet available through the configured feed. Remove the exceptions and update both selector overrides when patched releases become available. Do not replace selector overrides with name-nested overrides.
