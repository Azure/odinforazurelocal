...
- **fast-uri**: NEVER allow `fast-uri` to regress below version **4.1.1**. Earlier releases have four high-severity URI-parsing issues: path traversal via percent-encoded dot segments (GHSA-q3j6-qgpj-74h6 / CVE-2026-6321), host confusion via percent-encoded authority delimiters (GHSA-v39h-62p7-jpjc / CVE-2026-6322), failed IDN canonicalization (GHSA-4c8g-83qw-93j6 / CVE-2026-13676), and literal backslash authority delimiters (GHSA-v2hh-gcrm-f6hx / CVE-2026-16221). Pulled in transitively via `html-validate → ajv` and `stylelint → table → ajv`. The override in `package.json` (`"fast-uri": ">=4.1.1"`) must be maintained. If any dependency change or PR attempts to downgrade this, reject it. (Latest findings flagged July 2026 via Dependabot alerts #19 and #20.)
- **brace-expansion**: NEVER allow the `brace-expansion` **5.x** line to regress below version **5.0.7** or the **1.x** line below **1.1.16**. The version-selector overrides protect both consumers without forcing the eslint toolchain's 1.x dependency onto 5.x. GHSA-mh99-v99m-4gvg / CVE-2026-14257 affects all published versions through 5.0.7; the patched 5.0.8 release is not yet available through the package feed. `scripts/check-npm-audit.js` therefore temporarily permits only that exact advisory while continuing to block every other high/critical advisory. Remove the exception and update the override as soon as 5.0.8 is available through the feed. Do not replace the selectors with name-nested overrides: npm 10 and npm 11 resolve those differently and produce divergent lockfiles.
- **postcss**: NEVER allow `postcss` to regress below version **8.5.18**. Versions ≤ 8.5.17 permit path traversal when auto-loading a previous source map (GHSA-r28c-9q8g-f849). Pulled in transitively through `stylelint`; the override in `package.json` (`"postcss": ">=8.5.18"`) must be maintained.
